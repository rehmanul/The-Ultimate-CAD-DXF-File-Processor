class CorridorRouter {
    ...
    static generateBridgingCorridors(corridors, floorPlan, disconnectedComponents, options = {}) {
        ...
        const padding = 0.75;
        const ilots = Array.isArray(options.ilots) ? options.ilots : [];

        const overlapsAnyIlot = (corridor) => {
            if (!corridor || !ilots.length) return false;
            ...
            const inset = 0.02;
            const cLeft = cx + inset;
            const cRight = cx + cw - inset;
            const cTop = cy + inset;
            const cBottom = cy + ch - inset;
            if (cRight <= cLeft || cBottom <= cTop) return false;

            for (const ilot of ilots) {
                const ix = Number(ilot?.x);
                const iy = Number(ilot?.y);
                const iw = Number(ilot?.width);
                const ih = Number(ilot?.height);
                if (![ix, iy, iw, ih].every(Number.isFinite) || iw <= 0 || ih <= 0) continue;

                const iLeft = ix + inset;
                const iRight = ix + iw - inset;
                const iTop = iy + inset;
                const iBottom = iy + ih - inset;
                    ...
                    for (const nodeA of connectedComponent.nodes) {
                        for (const nodeB of unconnectedComponent.nodes) {
                            ...
                            if (distance < bestDistance) {
                                ...
                                if (bridge && !overlapsAnyIlot(bridge)) {
                                    bestBridge = bridge;
