...
class FacingRowDetector {
     ...
     * Corridor is strictly between the two rows (inner edges row1.maxY and row2.minY)
     * and spans the full horizontal overlap for one continuous band.
     */
    generateCorridorRecommendations(corridorWidth = 1.5) {
        const result = this.detectFacingRows();
        const recommendations = [];

        // Architectural rule: corridors are generated between adjacent facing rows,
        // not every possible row-pair (which creates overlapping/noisy corridors).
        ...
        for (let i = 0; i < rows.length - 1; i++) {
            ...
            const corridorPosition = relationship.corridorPosition;
            const minY = corridorPosition.minY; // row1.maxY
            const maxY = corridorPosition.maxY; // row2.minY
            const availableHeight = maxY - minY;
            if (availableHeight < 0.3) continue;

            const corridorHeight = Math.min(Math.max(0.8, Number(corridorWidth) || 1.2), availableHeight);
            const corridorY = minY + (availableHeight - corridorHeight) / 2;

            const leftMost = corridorPosition.x;
            const rightMost = corridorPosition.x + corridorPosition.width;
            const qualityScore = relationship.qualityScore;
            ...
            recommendations.push({
                ...
                area: corridorPosition.width * corridorHeight,
                connectsRows: [i, i + 1],
