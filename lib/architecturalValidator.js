...
class ArchitecturalValidator {
     ...
     */
    validate() {
        const roomIssues = this.rawPlan ? [] : this.validateRooms();
        const corridorIssues = this.rawPlan ? [] : this.validateCorridors();
        const egressIssues = this.validateEgressPaths();
        const deadEndIssues = this.rawPlan ? [] : this.validateDeadEnds();
        const travelDistanceIssues = this.rawPlan ? [] : this.validateTravelDistances();
        const ilotPlacementIssues = this.rawPlan ? [] : this.validateIlotPlacement();
    ...
    validateEgressPaths() {
        ...
        if (entrances.length === 0) {
            issues.push({
                type: 'egress-missing',
                severity: this.rawPlan ? 'warning' : 'error',
                message: 'No exits/entrances detected. At least one exit is required.',
                element: 'floor-plan',
                actual: 0,
                required: 1
            });
        } else if (entrances.length === 1 && !this.rawPlan) {
            issues.push({
                type: 'egress-single',
                severity: 'warning',
