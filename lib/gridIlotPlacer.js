...
class GridIlotPlacer {
    ...
    _detectUsableSpaces() {
        ...
        if (this.rooms && this.rooms.length > 0) {
            ...
            })).filter(s => s.area > 1); // Only rooms > 1m²
        }
    ...
    generateIlots(distribution, targetCount = 50) {
        ...
        for (const space of this.usableSpaces) {
            ...
            const ilots = this._placeInGrid(spaceSizes, layout, space.bounds);
    ...
    _calculateSizes(distribution, targetCount) {
        ...
        Object.entries(distribution).forEach(([range, percentage]) => {
            ...
            const count = Math.round(targetCount * percentage);
    ...
    _placeInGrid(sizes, layout, bounds = this.bounds) {
        const ilots = [];
        let sizeIndex = 0;
        ...
        for (let row = 0; row < layout.rows && sizeIndex < sizes.length; row++) {
            ...
            for (let col = 0; col < layout.cols && sizeIndex < sizes.length; col++) {
                ...
                if (this._isValidPosition(x, y, width, height, bounds, ilots)) {
                    const ilot = {
    ...
    _isValidPosition(x, y, width, height, spaceBounds = null, existingIlots = []) {
        const bounds = spaceBounds || this.bounds;
