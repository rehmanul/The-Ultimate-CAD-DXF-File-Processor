...
class GridIlotPlacer {
    ...
    _detectUsableSpaces() {
        ...
        if (this.rooms && this.rooms.length > 0) {
            ...
            if (roomSpaces.length > 0 && avgRoomArea >= 3) {
                console.log(`[Grid Placer] Found ${roomSpaces.length} valid rooms for placement (avg: ${avgRoomArea.toFixed(2)}m²)`);
                return roomSpaces;
            }
            console.log(`[Grid Placer] Rooms too small (avg: ${avgRoomArea.toFixed(2)}m²), using full floor bounds for îlot placement`);
        }

        console.log('[Grid Placer] No rooms or rooms too small, using full bounds');
        return [{
            bounds: this.bounds,
    ...
    _calculateGridLayout(sizes, bounds = this.bounds) {
        ...
        if (boundsWidth <= 0 || boundsHeight <= 0 || !Number.isFinite(boundsWidth) || !Number.isFinite(boundsHeight)) {
            console.warn(`[Grid Placer] Invalid bounds dimensions: {width: ${boundsWidth}, height: ${boundsHeight}}, using defaults`);
            // Fallback to floor bounds
            bounds = this.bounds;
        }

        const usableWidth = Math.max(2, (bounds.maxX - bounds.minX) - (2 * this.margin));
        const usableHeight = Math.max(2, (bounds.maxY - bounds.minY) - (2 * this.margin));

        const avgWidth = sizes.reduce((sum, s) => sum + s.width, 0) / Math.max(1, sizes.length);
        const avgHeight = sizes.reduce((sum, s) => sum + s.height, 0) / Math.max(1, sizes.length);

        // Calculate optimal grid to fit all ilots
        const totalIlots = sizes.length;
        const aspectRatio = usableWidth / usableHeight;

        // For extreme or invalid aspect ratios, use simple square grid
        let cols, rows;
        if (!Number.isFinite(aspectRatio) || aspectRatio <= 0) {
            // Invalid aspect ratio - use square grid
            cols = rows = Math.ceil(Math.sqrt(totalIlots));
        ...
        console.log(`[Grid Placer] Grid: ${rows} rows x ${cols} cols (aspect: ${aspectRatio.toFixed(2)}, space: ${usableWidth.toFixed(1)}x${usableHeight.toFixed(1)}m)`);

        return { rows, cols, avgWidth, avgHeight, usableWidth, usableHeight };
    }
