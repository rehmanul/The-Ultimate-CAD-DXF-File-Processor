...
class DXFProcessor {
    processParsedDXF(dxf) {
        ...
        // 🧠 INTELLIGENT DETECTION: Apply ML inference for missing elements
        const inferredData = this.applyIntelligentDetection(
            { walls, forbiddenZones, entrances, rooms, bounds: { minX: 0, minY: 0, maxX: width, maxY: height, width, height } },
            { walls, forbiddenZones, entrances }
        );
     ...
     */
    applyIntelligentDetection(floorPlan, data) {
        ...
        if (data.entrances.length === 0 && data.walls.length > 0) {
            console.log('[DXF Processor] ⚠️  No entrances detected. Using ML to infer...');

            // Convert wall polygons to line format for detector
            const wallLines = data.walls
                .filter(w => w.polygon && w.polygon.length >= 2)
                .map(w => ({
                    start: { x: w.polygon[0][0], y: w.polygon[0][1] },
                    end: { x: w.polygon[w.polygon.length - 1][0], y: w.polygon[w.polygon.length - 1][1] }
                }));

            const detectedDoors = IntelligentDetector.detectDoors(wallLines, floorPlan.bounds);

            if (detectedDoors.length > 0) {
                data.entrances.push(...detectedDoors);
                result.hasInferences = true;
                result.inferredEntrances = detectedDoors.length;
                result.confidence.entrances = 0.75;
                console.log(`[DXF Processor] ✅ Inferred ${detectedDoors.length} entrance(s) from wall gaps`);
            } else {
                console.log(`[DXF Processor] ℹ️  Could not infer entrances from wall patterns`);
            }
        }

        // If no forbidden zones detected, try to infer them
        if (data.forbiddenZones.length === 0 && floorPlan.rooms.length > 0) {
            console.log('[DXF Processor] ⚠️  No forbidden zones detected. Using ML to infer...');
            ...
            if (detectedElevators.length > 0) {
                data.forbiddenZones.push(...detectedElevators);
                result.hasInferences = true;
                result.inferredForbiddenZones = detectedElevators.length;
                result.confidence.forbiddenZones = 0.85;
                console.log(`[DXF Processor] ✅ Inferred ${detectedElevators.length} forbidden zone(s) (elevators/stairs)`);
            } else {
                console.log(`[DXF Processor] ℹ️  Could not infer forbidden zones from room patterns`);
            }
        }
