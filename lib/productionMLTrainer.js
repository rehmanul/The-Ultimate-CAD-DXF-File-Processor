...
let tf;
try {
    // Attempt to load the native bindings when available for better performance.
    // Wrapping the require in a resolve check lets us emit a concise warning instead of a full stack trace.
    require.resolve('@tensorflow/tfjs-node');
    tf = require('@tensorflow/tfjs-node');
} catch (error) {
    const isMissing = error && (error.code === 'MODULE_NOT_FOUND' || /Cannot find module/i.test(error.message || ''));
    if (isMissing) {
        console.warn('[@tensorflow/tfjs-node] native bindings not installed – continuing with @tensorflow/tfjs (CPU/WebGL). Install @tensorflow/tfjs-node for faster local inference.');
    } else {
        console.warn('[@tensorflow/tfjs-node] Failed to initialize native bindings, falling back to @tensorflow/tfjs. Details:', error && error.message ? error.message : error);
    }
