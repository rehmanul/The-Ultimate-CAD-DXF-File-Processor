...
class CADEntityClassifier {
    ...
    classifyEntity(entity) {
        ...
        const colorRaw = entity.colorRaw !== undefined ? entity.colorRaw : (entity.color || 0);
        ...
        // Fix 6: Door openings (cyan rectangles) -> make them render visible
        // Fix 5: Parse dashed lines as zone boundaries
        if (color === 4 || color === 0x00FFFF || colorRaw === 4 || layer.includes('DOOR') || layer.includes('CYAN')) {
            return { type: 'entrance', confidence: 0.95 };
        }
        
        const lineType = (entity.lineType || '').toLowerCase();
        if (lineType.includes('dashed') || layer.includes('DASHED')) {
            return { type: 'forbidden', confidence: 0.95 };
        }
