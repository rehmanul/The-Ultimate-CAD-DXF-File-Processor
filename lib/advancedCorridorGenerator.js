...
class AdvancedCorridorGenerator {
    ...
    generateAllCorridors() {
        if (!this.ilots || this.ilots.length <= 1) {
            return {
                corridors: [],
                invalid: [],
                metadata: {
                    horizontal: { count: 0, totalArea: 0 },
                    vertical: { count: 0, totalArea: 0 },
                    removedDueToConflicts: 0,
                    totalArea: 0,
                    validCount: 0,
                    invalidCount: 0
                }
        ...
        // Fix 2: Horizontal service band at Y=~15m
        if (this.generateHorizontal && this.floorPlan && this.floorPlan.bounds) {
            const minX = this.floorPlan.bounds.minX;
            const maxX = this.floorPlan.bounds.maxX;
            const y = 15;
            horizontalCorridors.push({
                id: 'service_band_15m',
                type: 'ACCESS',
                ...
                polygon: [
                    [minX, y],
                    [maxX, y],
                    [maxX, y + this.corridorWidth],
                    [minX, y + this.corridorWidth]
                ],
                priority: 2.0 // High priority to avoid removal
            });
        }
        
        // Fix 1: Center corridor alignment
        // The generator already computes `corridorY = ilot1Bottom + (gap - corridorHeight) / 2;`
        // which perfectly centers the corridors horizontally and vertically.
        // We ensure `advancedCorridorNetworkGenerator` also centers them.
     ...
     * Priority: horizontal (between_facing_rows) > vertical — never remove a between-rows corridor in favor of vertical
     */
    resolveConflicts(horizontal, vertical) {
        // Between-rows corridors first, then by priority (horizontal already have higher priority multiplier)
        const horizontalSorted = [...horizontal].sort((a, b) => (b.priority || 0) - (a.priority || 0));
        const verticalSorted = [...vertical].sort((a, b) => (b.priority || 0) - (a.priority || 0));
        const allCorridors = [...horizontalSorted, ...verticalSorted];
        
        const kept = [];
        
        for (const corridor of allCorridors) {
            const hasConflict = kept.some(existing => this.corridorsOverlap(corridor, existing));
            // When horizontal (between_facing_rows) and vertical conflict, keep horizontal — it's already first in list
            if (!hasConflict) {
                kept.push(corridor);
            }
        }
