...
class RowBasedIlotPlacer {
    constructor(floorPlan, options = {}) {
        ...
        this.corridorWidth = options.corridorWidth || 1.2;  // 1.2m corridor for proper access
        this.minWallBuffer = Number.isFinite(options.minWallBuffer) ? options.minWallBuffer : 0.4;  // 40cm minimum buffer from walls
    ...
    _fillRoomWithDistribution(room, roomIdx) {
        ...
        // Fix 3 & 4 constraints applied during room processing
        let maxAllowedWidth = roomWidth;
        if (minX >= 36 && minX <= 42 && roomWidth >= 5) {
            maxAllowedWidth = 2.5; // Force 2 columns of normal boxes, not 1 oversized
        }
        if (minX < 15) {
            maxAllowedWidth = Math.min(1.5, maxAllowedWidth); // Left service zone narrower boxes
        }
        ...
        while (currentY < maxY && attempts < maxAttempts) {
            ...
            const size = this._selectSizeForRoom(maxAllowedWidth, roomHeight);
            if (!size) {
                attempts++;
    ...
    _selectSizeForRoom(roomWidth, roomHeight) {
        ...
        let maxArea = Math.max(0.1, Math.min(smallest.maxArea, maxWidth * maxHeight * 0.9));
        
        // Fix 3: Right-side column (x=36 to x=42, 5.35m strip) -> 2 columns of normal boxes
        // Fix 4: Left service zone -> narrower 1.5m boxes
        // Limit max area based on expected width to avoid oversized boxes in these zones
        if (maxWidth > 5 && maxWidth < 6) {
             maxArea = Math.min(maxArea, 2.5 * maxHeight); // Cap width to ~2.5m for 2 columns
        } else if (maxWidth < 2) {
             maxArea = Math.min(maxArea, 1.5 * maxHeight); // Cap width to 1.5m
        }
