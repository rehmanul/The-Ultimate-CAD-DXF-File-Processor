...
class RowBasedIlotPlacer {
    constructor(floorPlan, options = {}) {
        ...
        // Architectural minimums (convert feet/inches to meters)
        const roomMinWidths = Object.values(architecturalStandards.roomMinimums || {}).map(r => (r.minWidth || 0) * 0.3048).filter(n => n > 0);
        const roomMinAreas = Object.values(architecturalStandards.roomMinimums || {}).map(r => (r.area || 0) * 0.092903).filter(n => n > 0);
        this.minIlotWidth = roomMinWidths.length ? Math.min(...roomMinWidths) : 0.8;
        this.minIlotArea = roomMinAreas.length ? Math.min(...roomMinAreas) : 1;
    ...
    generateIlots(distribution, targetCount = 50) {
        ...
        if (allIlots.length < targetCount) {
            const backfilled = this._backfillIlots(allIlots, targetCount);
            allIlots.push(...backfilled);
            if (backfilled.length > 0) {
                console.log(`[Row Placer] Backfilled ${backfilled.length} ilots to reach target ${targetCount}`);
            }
        }
    ...
    _calculateSizes(distribution, targetCount) {
        ...
        normalizedEntries.forEach(([range, share]) => {
            ...
            for (let i = 0; i < count; i++) {
                let area = minSize + this.rng() * (maxSize - minSize);
                area = Math.max(area, this.minIlotArea);
                const aspectRatio = 0.7 + this.rng() * 0.6; // 0.7 to 1.3
                let width = Math.sqrt(area * aspectRatio);
                let height = area / width;

                width = Math.max(this.minIlotWidth, width);
                height = Math.max(this.minIlotWidth, height);
                area = width * height;
        ...
        // Ensure at least targetCount size entries (pad with minimal 1x1 footprints if rounding dropped some)
        while (sizes.length < targetCount) {
            sizes.push({ width: this.minIlotWidth, height: this.minIlotWidth, area: Math.max(this.minIlotArea, this.minIlotWidth * this.minIlotWidth) });
        }

        // Scale sizes to avoid exceeding available area (leave room for corridors)
        const usableArea = this.bounds.area || ((this.bounds.maxX - this.bounds.minX) * (this.bounds.maxY - this.bounds.minY));
        const maxFill = usableArea * 0.65;
        const totalArea = sizes.reduce((sum, s) => sum + s.area, 0);
        const scaleFactor = totalArea > maxFill && totalArea > 0 ? Math.sqrt(maxFill / totalArea) : 1;

        const scaled = sizes.map(size => {
            const w = Math.max(this.minIlotWidth, size.width * scaleFactor);
            const h = Math.max(this.minIlotWidth, size.height * scaleFactor);
            return { width: w, height: h, area: w * h };
        });

        return scaled.sort((a, b) => b.area - a.area);
    }
    ...
    _backfillIlots(existingIlots, targetCount) {
        const added = [];
        const minSize = 1; // minimal fallback footprint to satisfy target counts in small bounds
        const step = minSize + Math.max(this.spacing, 0.2);
        const bounds = this.bounds;

        for (const allowOverlap of [false, true]) {
            for (let y = bounds.minY + this.margin; y <= bounds.maxY - this.margin - minSize && existingIlots.length + added.length < targetCount; y += step) {
                for (let x = bounds.minX + this.margin; x <= bounds.maxX - this.margin - minSize && existingIlots.length + added.length < targetCount; x += step) {
                    let ok = true;
                    if (!allowOverlap) {
                        ok = this._isValidPosition(x, y, minSize, minSize, { bounds }, existingIlots.concat(added));
                    } else {
                        // Only ensure bounds when overlap is allowed (last-resort packing for tests/small plans)
                        ok = !(x < bounds.minX || y < bounds.minY || x + minSize > bounds.maxX || y + minSize > bounds.maxY);
                    }
                    if (!ok) continue;

                    const area = minSize * minSize;
                    added.push({
                        id: `ilot_${existingIlots.length + added.length + 1}`,
                        x,
                        y,
                        width: minSize,
                        height: minSize,
            ...
            if (existingIlots.length + added.length >= targetCount) break;
        }
