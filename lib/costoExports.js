...
`;
    ...
    async drawEnhancedFloorPlanToPDF(page, floorPlan, solution, scale, offsetX, offsetY, options = {}) {
        ...
        // Draw walls (thin gray lines - matching reference "Tôle Blanche" style)
        const ToleBlancheColor = rgb(0.42, 0.45, 0.5);
        ...
        boxes.forEach(box => {
            ...
            // Draw box number (ID) prominently - floor-based numbering like 101, 102
            if (options.showBoxNumbers !== false && box.id) {
                const boxNumber = box.displayNumber || box.id;
                const numText = String(boxNumber);
                const numFontSize = Math.max(10, Math.min(14, w * 0.2)); // Larger font for numbers
                const numTextWidth = numText.length * numFontSize * 0.6;

                // Draw white background for number (for visibility)
                const padding = 3;
                page.drawRectangle({
                    x: x + w / 2 - numTextWidth / 2 - padding,
                    y: y + h / 2 + numFontSize / 2 - padding,
                    width: numTextWidth + padding * 2,
                    height: numFontSize + padding * 2,
                    color: COLORS.WHITE,
                    opacity: 0.8
                });
            ...
            if (options.showUnitLabels) {
                ...
                page.drawText(labelText, {
                    x: x + w / 2 - textWidth / 2,
                    y: y + h / 2 - fontSize / 3 - (options.showBoxNumbers !== false ? 10 : 0), // Offset if showing numbers
                    size: fontSize,
                    font: boldFont,
        ...
        // ── RADIATORS: Red zigzag polylines along perimeter walls ─────
        // Reference: "Radiateur" = red zigzag along exterior wall segments
        const RADIATOR_RED = rgb(0.85, 0, 0.08);
        const radiatorData = solution.radiators || [];
        if (radiatorData.length > 0) {
            radiatorData.forEach(radiator => {
                const rPath = radiator.path || [];
                for (let ri = 0; ri < rPath.length - 1; ri++) {
                    page.drawLine({
                        start: {
                            x: offsetX + rPath[ri].x * scale,
                            y: offsetY + rPath[ri].y * scale
                        },
                        end: {
                            x: offsetX + rPath[ri + 1].x * scale,
                            y: offsetY + rPath[ri + 1].y * scale
                        },
                        thickness: 1.0,
                        color: RADIATOR_RED
                    });
            ...
            // Fallback: generate radiator zigzag along floor perimeter bounds
            ...
            perimeterEdges.forEach(edge => {
                ...
                const enx = edot >= 0 ? en1x : -en1x;
                const eny = edot >= 0 ? en1y : -en1y;
                const halfW = radWL / 2;
                const numPts = Math.max(4, Math.floor(elen / halfW) + 1);
                const pts = [];
                for (let ri = 0; ri <= numPts; ri++) {
                    const t = Math.min(ri * halfW / elen, 1.0);
                    const bpx = edge.x1 + edx * t + enx * radOff;
                    const bpy = edge.y1 + edy * t + eny * radOff;
                    const zs = (ri % 2 === 0) ? 1 : -1;
                    pts.push({
                        x: offsetX + (bpx + enx * zs * radAmp) * scale,
                        y: offsetY + (bpy + eny * zs * radAmp) * scale
                    });
                }
                for (let ri = 0; ri < pts.length - 1; ri++) {
                    page.drawLine({
                        start: pts[ri], end: pts[ri + 1],
                        thickness: 1.0, color: RADIATOR_RED
                    });
                }
            });
        }
