...
class CostoExports {
    ...
    _shouldRenderEnvelope(floorPlan) {
        ...
        // Always render envelope - even synthetic ones provide important building context
        // Previously skipped synthetic envelopes, but the reference output shows the
        // building outline is always important for structural context
...
`;
    ...
    async drawEnhancedFloorPlanToPDF(page, floorPlan, solution, scale, offsetX, offsetY, options = {}) {
        ...
        // Draw with light yellow fill + cross-hatch pattern + visible border
        const RESTRICTED_FILL = rgb(1.0, 0.95, 0.75);      // Light yellow
        const RESTRICTED_BORDER = rgb(0.85, 0.65, 0.0);     // Amber border
        const RESTRICTED_HATCH = rgb(0.9, 0.75, 0.3);       // Hatch lines
        if (floorPlan.forbiddenZones) {
            floorPlan.forbiddenZones.forEach(zone => {
                if (zone.polygon && zone.polygon.length >= 3) {
                    ...
                    // Bounding box for fill + hatching
                    ...
                    // Light yellow fill (approximate with rectangle for convex zones)
                    if (points.length === 4 || points.length === 3) {
                        page.drawRectangle({
                            x: zMinX, y: zMinY,
                            width: zMaxX - zMinX, height: zMaxY - zMinY,
                            color: RESTRICTED_FILL, opacity: 0.5
                        });
                    }

                    // Cross-hatch pattern (diagonal lines)
                    const hatchSpacing = 6;
                    for (let hx = zMinX; hx <= zMaxX + (zMaxY - zMinY); hx += hatchSpacing) {
                        page.drawLine({
                            start: { x: hx, y: zMinY },
                            end: { x: hx - (zMaxY - zMinY), y: zMaxY },
                            thickness: 0.3, color: RESTRICTED_HATCH, opacity: 0.6
                        });
                    }
                    ...
                    for (let i = 0; i < points.length; i++) {
                        const p1 = points[i];
                        const p2 = points[(i + 1) % points.length];
                        page.drawLine({ start: p1, end: p2, thickness: 1.2, color: RESTRICTED_BORDER });
                    }

                    // Label "Zone interdite" in the center
                    const zcx = (zMinX + zMaxX) / 2;
                    const zcy = (zMinY + zMaxY) / 2;
                    if (zMaxX - zMinX > 20 && zMaxY - zMinY > 10) {
                        page.drawText('Zone interdite', {
                            x: zcx - 25, y: zcy - 3,
                            size: 6, font, color: RESTRICTED_BORDER
                        });
                    }
                }
            });
        ...
        // ── ENTRANCES ─────────────────────────────────────────────────────
        // Draw with colored markers and clear labels (red for entrances)
        const ENTRANCE_COLOR = rgb(0.85, 0.15, 0.1);
        if (floorPlan.entrances) {
            floorPlan.entrances.forEach(entrance => {
                if (entrance.start && entrance.end) {
                    const x1 = offsetX + entrance.start.x * scale;
                    const y1 = offsetY + entrance.start.y * scale;
                    const x2 = offsetX + entrance.end.x * scale;
                    const y2 = offsetY + entrance.end.y * scale;
                    const midX = (x1 + x2) / 2;
                    const midY = (y1 + y2) / 2;
                    ...
                    page.drawLine({
                        ...
                        thickness: 2.5, color: ENTRANCE_COLOR
                    });

                    // Small red triangles at endpoints
                    const triSize = 4;
                    const dx = x2 - x1, dy = y2 - y1;
                    const len = Math.hypot(dx, dy) || 1;
                    const nx = -dy / len * triSize, ny = dx / len * triSize;
                    page.drawLine({ start: { x: midX, y: midY }, end: { x: midX + nx, y: midY + ny }, thickness: 2, color: ENTRANCE_COLOR });
                    page.drawLine({ start: { x: midX, y: midY }, end: { x: midX - nx, y: midY - ny }, thickness: 2, color: ENTRANCE_COLOR });

                    // Label
                    const label = entrance.label || 'Entree';
                    page.drawText(label, {
                        x: midX - label.length * 2,
                        y: midY + 6,
                        size: 6, font: boldFont, color: ENTRANCE_COLOR
                    });
                }
            });
        }
        ...
        // ── EXITS ─────────────────────────────────────────────────────────
        // Draw with green markers and labels
        const EXIT_COLOR = rgb(0.1, 0.6, 0.15);
        if (floorPlan.exits) {
            floorPlan.exits.forEach(exit => {
                if (exit.start && exit.end) {
                    const x1 = offsetX + exit.start.x * scale;
                    const y1 = offsetY + exit.start.y * scale;
                    const x2 = offsetX + exit.end.x * scale;
                    const y2 = offsetY + exit.end.y * scale;
                    const midX = (x1 + x2) / 2;
                    const midY = (y1 + y2) / 2;
                    ...
                    page.drawLine({
                        start: { x: x1, y: y1 }, end: { x: x2, y: y2 },
                        thickness: 2.5, color: EXIT_COLOR
                    });
                    ...
                    page.drawLine({ start: { x: midX, y: midY }, end: { x: midX + nx, y: midY + ny }, thickness: 2, color: EXIT_COLOR });

                    page.drawText('Sortie', {
                        x: midX - 10, y: midY + 7,
                        size: 6, font: boldFont, color: EXIT_COLOR
                    });
                }
            });
        }
        ...
        boxes.forEach(box => {
            ...
            // Area + dimension labels (matching reference: "1.50x2.30" + "3.45m²")
            if (options.showDimensions && box.width && box.height) {
                const dimFontSize = Math.max(4, Math.min(6, w * 0.08));

                // Dimension text "WxH" inside box (top area)
                const dimText = `${box.width.toFixed(2)}x${box.height.toFixed(2)}`;
                const dimTextWidth = dimText.length * dimFontSize * 0.5;
                page.drawText(dimText, {
                    x: x + w / 2 - dimTextWidth / 2,
                    y: y + h - dimFontSize - 2,
                    size: dimFontSize,
                    font,
                    color: DIM_COLOR
                ...
                // Area text below dimension inside box
                const areaText = `${area.toFixed(2)}m\u00B2`;
                const areaTextWidth = areaText.length * dimFontSize * 0.5;
                ...
                // Width dimension along bottom edge (outside box)
                const wLabel = `${box.width.toFixed(2)}`;
                page.drawText(wLabel, {
                    x: x + w / 2 - wLabel.length * 2,
                    y: y - 7,
                    size: 4.5,
                    font,
                ...
                // Height dimension along left edge (outside box)
                const hLabel = `${box.height.toFixed(2)}`;
                page.drawText(hLabel, {
                    x: x - hLabel.length * 3 - 2,
                    y: y + h / 2 - 2,
                    size: 4.5,
                    font,
                    color: COLORS.DARK_GREEN
        ...
        // ── ROW-PERIMETER RADIATORS (auto-generated from box rows) ───────
        // When no explicit radiator data, detect box rows and draw zigzags
        // along the outer edges of each row strip (matching reference style)
        if (useRowZigzags && radiatorData.length === 0 && boxes.length > 4) {
            // Group boxes by approximate y-position into rows
            const rowTol = 0.3; // metres tolerance for same-row grouping
            const rowMap = new Map();
            boxes.forEach(box => {
                const by = Math.round(box.y / rowTol) * rowTol;
                if (!rowMap.has(by)) rowMap.set(by, []);
                rowMap.get(by).push(box);
            });

            // Find row pairs (two rows close together = strip with corridor between)
            const rowYs = [...rowMap.keys()].sort((a, b) => a - b);
            const processedRows = new Set();

            rowYs.forEach(ry => {
                if (processedRows.has(ry)) return;
                const rowBoxes = rowMap.get(ry);
                if (!rowBoxes || rowBoxes.length < 2) return;

                const rowMinX = Math.min(...rowBoxes.map(b => b.x));
                const rowMaxX = Math.max(...rowBoxes.map(b => b.x + b.width));
                const rowMinY = Math.min(...rowBoxes.map(b => b.y));
                const rowMaxY = Math.max(...rowBoxes.map(b => b.y + b.height));

                processedRows.add(ry);

                // Draw zigzag along the top edge of this row
                const topZig = makeZigzagPath(rowMinX, rowMaxY + 0.08, rowMaxX, rowMaxY + 0.08);
                drawRadiatorPolyline(topZig, RAD_LINE_THICKNESS * 0.8, RADIATOR_RED);

                // Draw zigzag along the bottom edge of this row
                const bottomZig = makeZigzagPath(rowMinX, rowMinY - 0.08, rowMaxX, rowMinY - 0.08);
                drawRadiatorPolyline(bottomZig, RAD_LINE_THICKNESS * 0.8, RADIATOR_RED);
            });
        }
