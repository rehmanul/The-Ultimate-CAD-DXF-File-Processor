...
`;
    ...
    async drawFloorPlanToPDF(page, floorPlan, solution, scale, offsetX, offsetY, options = {}) {
        ...
        if (options.scale) {
            this.drawScaleAnnotation(page, baseOffsetX - 30, baseOffsetY, options.scale, font, options.floorLabel);
        }
    ...
    async drawLegend(page, x, y, width, height, options = {}) {
        ...
        const legendItems = [
            ...
            },
            {
                ...
                lineStyle: 'dashed',
                color: rgb(0.533, 0, 0.082),  // Brown/dark red per spec (RGB 136 0 21)
                thickness: 1
            }
        ];
...
`;
    ...
    async drawEnhancedFloorPlanToPDF(page, floorPlan, solution, scale, offsetX, offsetY, options = {}) {
        ...
        const corridorColor = COLORS.RED;
        const boxesForRows = solution.boxes || [];
        const rowZigzags = [];
        if (useRowZigzags && boxesForRows.length > 0) {
            const heights = boxesForRows.map(b => b.height).filter(h => Number.isFinite(h) && h > 0).sort((a, b) => a - b);
            const medianHeight = heights.length ? heights[Math.floor(heights.length / 2)] : 1.2;
            const rowTolerance = Math.max(0.6, medianHeight * 0.6);
            const rows = new Map();

            boxesForRows.forEach((box) => {
                const centerY = box.y + box.height / 2;
                const key = Math.round(centerY / rowTolerance) * rowTolerance;
                if (!rows.has(key)) rows.set(key, []);
                rows.get(key).push(box);
            ...
            Array.from(rows.values()).forEach((rowBoxes) => {
                ...
                for (let px = minX; px <= maxX; px += frequency) {
                    const offset = peak ? amplitude : -amplitude;
                    points.push({ x: offsetX + px * scale, y: offsetY + (centerY + offset) * scale });
                    peak = !peak;
                }
                rowZigzags.push(points);
            });
        ...
        if (useRowZigzags && rowZigzags.length > 0) {
            ...
            // Draw corridors (red zigzag lines like reference)
            const corridors = solution.corridors || [];
            corridors.forEach(corridor => {
                let x = corridor.x;
                let y = corridor.y;
            ...
            const isHorizontal = w >= h;
            const amplitude = Math.max(0.15, Math.min(0.6, (isHorizontal ? h : w) * 0.35));
            const frequency = Math.max(0.6, (isHorizontal ? w : h) / 18);
            const points = [];
            ...
            if (isHorizontal) {
                ...
                for (let py = startY; py <= endY; py += frequency) {
                    const offset = peak ? amplitude : -amplitude;
                    points.push({ x: offsetX + (centerX + offset) * scale, y: offsetY + py * scale });
                    peak = !peak;
                }
            }
            ...
            for (let i = 0; i < points.length - 1; i++) {
                ...
                if (corridor.width && Math.max(w, h) > 5) {
                    const midX = offsetX + (x + w / 2) * scale;
                    const midY = offsetY + (y + h / 2) * scale;
