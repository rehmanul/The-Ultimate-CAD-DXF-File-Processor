...
`;
    ...
    async drawFloorPlanToPDF(page, floorPlan, solution, scale, offsetX, offsetY, options = {}) {
        ...
        if (!hideRawWalls) {
            ...
            // ── Connected perimeter border: close gaps between wall endpoints ──
            // Draw a continuous rectangle at the building boundary so every edge is visually connected
            const bb = floorPlan.buildingBounds || floorPlan.bounds;
            if (bb) {
                const bx = offsetX + num(bb.minX) * scale;
                const by = offsetY + num(bb.minY) * scale;
                const bw = (num(bb.maxX) - num(bb.minX)) * scale;
                const bh = (num(bb.maxY) - num(bb.minY)) * scale;
                if (bw > 0 && bh > 0) {
                    page.drawRectangle({
                        x: bx, y: by, width: bw, height: bh,
                        borderColor: rgb(0.38, 0.4, 0.45),
                        borderWidth: 1.0
                    });
                }
            }
        ...
        boxes.forEach(box => {
            ...
            // ── Light blue fill (matching COSTO reference) ──
            page.drawRectangle({
        ...
        // Default mode: green coil on perimeter boxes only.
        {
            ...
            if (isRefMode) {
                ...
                boxes.forEach(box => {
                    ...
                    // ── Radiator coils on ALL boxes (wall-facing edge) ──
        ...
        // Draw circulation: green arrows (reference "ligne circulation")
        const circulationPaths = Array.isArray(solution.circulationPaths) ? solution.circulationPaths : [];
        const corridors = solution.corridors || [];
        // Green arrows when greenArrows or showCirculationArrows is true, otherwise dark maroon
        const flowArrowColor = (options.greenArrows || options.showCirculationArrows) 
            ? rgb(0.15, 0.6, 0.15)  // GREEN arrows like reference
            : COLORS.circulation;    // dark maroon fallback
         ...
         * - Draw green arrows along the corridor (reference style)
         * - Skip arrows inside junction clear-zones for clean intersections
         */
        const drawSegmentJunctionAware = (p1, p2, jType) => {
            ...
            // Always draw arrows when showCirculationArrows is true
            if (showCirculationArrows) {
                // Arrows spaced along corridor
                for (let d = spacing * 0.5; d <= length - spacing * 0.3; d += spacing) {
                    const t = d / length;
                ...
                // Dead-end arrow at end of corridor
                ...
                // Fallback: draw red dashes when arrows not enabled
