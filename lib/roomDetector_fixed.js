class RoomDetector {
    ...
    detectRooms(walls, entrances, forbiddenZones, bounds) {
        if (!walls || walls.length === 0) return [];

        // Basic closed polygon detection
        let rooms = [];
        console.log(`[RoomDetector] Searching for closed polygons in ${walls.length} walls...`);
        ...
        console.log(`[RoomDetector] Segmenting open areas...`);
        const segmentedPolygons = this.segmentOpenAreas(walls, entrances, forbiddenZones, bounds);
        console.log(`[RoomDetector] Found ${segmentedPolygons.length} segmented regions`);
        ...
        uniquePolygons.forEach((polygon, index) => {
    ...
    segmentOpenAreas(walls, entrances, forbiddenZones, bounds) {
        ...
        console.log(`[RoomDetector] Wall graph built: ${graph.nodes.length} nodes, ${graph.edges.length} edges`);

        const voronoiRegions = this.generateVoronoiRegions(graph, bounds);
        console.log(`[RoomDetector] Generated ${voronoiRegions.length} Voronoi regions`);
        ...
        return voronoiRegions.filter(region => {
            ...
            if (intersectsEntrance) {
                // console.log(`[RoomDetector] Region rejected (intersects entrance)`);
                return false;
            }
            if (intersectsForbidden) {
                // console.log(`[RoomDetector] Region rejected (intersects forbidden)`);
                return false;
            }
            return true;
        });
    ...
    tracePolygon(segments, startIndex, used, maxSteps = 200) {
        const polygon = [];
        // Increased tolerance to 0.5 to match gap closing and imperfect CAD
        const tolerance = 0.5;
