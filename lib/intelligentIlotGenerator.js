class IntelligentIlotGenerator {
    ...
    isValidPosition(ilot, position, placedIlots, minDistance) {
        ...
        if (position.x >= 36 && position.x <= 42) {
            // Force normal box width (max ~2.65m) to ensure 2 columns fit in the 5.35m strip
            if (ilot.width > 2.65) return false;
        }

        // Fix 4: Left service zone -> narrower 1.5m boxes between room clusters
        if (position.x < 15) {
            if (ilot.width > 1.5) return false;
        }
