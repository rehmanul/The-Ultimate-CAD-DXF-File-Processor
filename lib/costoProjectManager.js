    ...
    ensureProjectsDir() {
        try {
            ...
            if (error.code === 'EACCES' || error.code === 'EPERM') {
                console.warn(`[CostoProjectManager] Cannot create ${this.projectsDir}, using fallback location`);
                // Use a writable location (e.g., /tmp or process.cwd())
                ...
                try {
                    ...
                    this.projectsDir = path.join('/tmp', 'costo-projects');
                    try {
                        if (!fs.existsSync(this.projectsDir)) {
                            fs.mkdirSync(this.projectsDir, { recursive: true });
                        }
                        console.warn(`[CostoProjectManager] Using /tmp fallback: ${this.projectsDir}`);
                    } catch (tmpError) {
                        console.error('[CostoProjectManager] Cannot create projects directory anywhere:', tmpError.message);
                        // Don't throw - allow server to start, operations will fail gracefully
                    }
                }
            } else {
                console.error('[CostoProjectManager] Error creating directory:', error.message);
