...
class CostoProLayoutEngine {
    constructor(floorPlan, options = {}) {
        ...
        this.autoRemoveConnectorBlockers = options.autoRemoveConnectorBlockers === true;
        this.connectorBlockerEvictionLimit = Math.max(
            0,
            Math.min(4, Number(options.connectorBlockerEvictionLimit) || 0)
        );
    ...
    _linkDisconnectedCorridors(corridors, units) {
        ...
        const canEvictBlockers = this.autoRemoveConnectorBlockers === true &&
            this.connectorBlockerEvictionLimit > 0 &&
            Array.isArray(units);
        ...
        const overlapMetrics = (rect, unit) => {
            const ox = Math.max(0, Math.min(rect.x + rect.width, unit.x + unit.width) - Math.max(rect.x, unit.x));
            const oy = Math.max(0, Math.min(rect.y + rect.height, unit.y + unit.height) - Math.max(rect.y, unit.y));
            const overlapArea = ox * oy;
            const unitArea = Math.max(0.0001, Number(unit.width) * Number(unit.height));
            return {
                overlapArea,
                overlapRatio: overlapArea / unitArea
            };
        };
        const borderMargin = Math.max(0.9, cw * 0.85);
        const findBlockingUnits = (rect) => {
            const blockers = [];
            for (const u of (units || [])) {
                if (!(rect.x < u.x + u.width && rect.x + rect.width > u.x && rect.y < u.y + u.height && rect.y + rect.height > u.y)) continue;
                const metrics = overlapMetrics(rect, u);
                if (metrics.overlapArea < 0.08 && metrics.overlapRatio < 0.08) continue;
                const nearBorder =
                    u.x <= this.bounds.minX + borderMargin ||
                    u.y <= this.bounds.minY + borderMargin ||
                    (u.x + u.width) >= this.bounds.maxX - borderMargin ||
                    (u.y + u.height) >= this.bounds.maxY - borderMargin;
                if (nearBorder) continue;
                blockers.push({
                    unit: u,
                    overlapArea: metrics.overlapArea,
                    overlapRatio: metrics.overlapRatio,
                    area: Number(u.width) * Number(u.height)
                });
            }
            blockers.sort((a, b) => {
                if (Math.abs(b.overlapArea - a.overlapArea) > 1e-6) return b.overlapArea - a.overlapArea;
        ...
        let skippedUnsafeConnectors = 0;
        const evictedConnectorUnits = [];

        const tryEvictConnectorBlockers = (candidate) => {
            if (!canEvictBlockers) return false;
            if (!hitsUnit(candidate.x, candidate.y, candidate.width, candidate.height)) return true;

            const remainingBudget = this.connectorBlockerEvictionLimit - evictedConnectorUnits.length;
            if (remainingBudget <= 0) return false;
            const span = Math.max(candidate.width, candidate.height);
            if (span < Math.max(3.0, cw * 2.2)) return false;

            const blockers = findBlockingUnits(candidate);
            if (blockers.length === 0) return false;

            const removed = [];
            for (const blocker of blockers) {
                if (removed.length >= remainingBudget) break;
                const idx = units.indexOf(blocker.unit);
                if (idx < 0) continue;
                units.splice(idx, 1);
                removed.push({
                    idx,
                    unit: blocker.unit,
                    overlapArea: blocker.overlapArea,
                    overlapRatio: blocker.overlapRatio
                });
                if (!hitsUnit(candidate.x, candidate.y, candidate.width, candidate.height)) {
                    evictedConnectorUnits.push(...removed.map((r) => r.unit));
                    const removedLabels = removed.map((r) => (
                        `${r.unit.id || 'unit'}#${r.unit.displayNumber || '?'}`
                    ));
                    console.log(
                        `[CostoProLayout v4] Connector unblock: evicted ${removed.length} blocker unit(s)`,
                        {
            ...
            removed.sort((a, b) => b.idx - a.idx).forEach((r) => units.splice(r.idx, 0, r.unit));
            return false;
        };
        ...
        while (maxIterations-- > 0) {
            ...
            for (const conn of connectors) {
                if (tryEvictConnectorBlockers(conn)) {
                    result.push(conn);
                    added++;
                } else {
                    // Try narrow version
                    ...
                    if (tryEvictConnectorBlockers(narrow)) {
                        result.push(narrow);
                        added++;
                    }
                }
            ...
            if (added === 0) {
                const blockedKey = bestA < bestB ? `${bestA}|${bestB}` : `${bestB}|${bestA}`;
                blockedPairs.add(blockedKey);
                skippedUnsafeConnectors += connectors.length;
                const skippedDiagnostics = connectors.map((conn) => ({
                    direction: conn.direction || ((conn.width || 0) >= (conn.height || 0) ? 'horizontal' : 'vertical'),
                    x: Number(conn.x.toFixed(3)),
                    y: Number(conn.y.toFixed(3)),
                    width: Number(conn.width.toFixed(3)),
                    height: Number(conn.height.toFixed(3))
                }));
                console.log(
                    `[CostoProLayout v4] Connector skipped (would overlap units) ` +
                    `(pair=${bestA}->${bestB}, dist=${bestDist.toFixed(3)}, blockedPairs=${blockedPairs.size})`,
                    skippedDiagnostics
                );
                continue;
            }
        ...
        const comps = findComponents(result);
        if (evictedConnectorUnits.length > 0) {
            console.log(`[CostoProLayout v4] Connector unblock summary: evicted ${evictedConnectorUnits.length} unit(s) to preserve hallway continuity`);
        }
        console.log(
            `[CostoProLayout v4] Linked corridors: ${result.length} total, ${comps.length} component(s), ` +
            `skippedUnsafeConnectors=${skippedUnsafeConnectors}`
        );
        return result;
    }
    ...
    _corridorCoverageNeedsSupplement(units, corridors) {
        const unitBounds = this._computeRectBounds(units);
        const corridorBounds = this._computeRectBounds(corridors);
        if (!unitBounds || !corridorBounds) return { needsSupplement: false, unitBounds, corridorBounds };

        const xRatio = corridorBounds.width / Math.max(0.001, unitBounds.width);
        const yRatio = corridorBounds.height / Math.max(0.001, unitBounds.height);
        const lowVerticalSpan = yRatio < 0.65;
        const lowHorizontalSpan = xRatio < 0.65;
        const clippedTop = corridorBounds.maxY < (unitBounds.maxY - Math.max(1.8, this.boxDepth * 0.9));
        const clippedBottom = corridorBounds.minY > (unitBounds.minY + Math.max(1.8, this.boxDepth * 0.9));
        const tooFewCorridors = Array.isArray(corridors) && corridors.length < 28;

        return {
            needsSupplement: lowVerticalSpan || lowHorizontalSpan || clippedTop || clippedBottom || tooFewCorridors,
            xRatio,
            yRatio,
            lowVerticalSpan,
            lowHorizontalSpan,
    ...
    _buildUnitGapCorridorGrid(units, existingCorridors = []) {
        const unitRects = (Array.isArray(units) ? units : [])
            .map((u) => ({
                x: Number(u?.x),
                y: Number(u?.y),
                width: Number(u?.width),
            ...
            .filter((u) => [u.x, u.y, u.width, u.height].every(Number.isFinite) && u.width > 0 && u.height > 0);
        if (unitRects.length < 20) return [];

        const b = this.bounds || { minX: 0, minY: 0, maxX: 0, maxY: 0 };
        ...
        const cw = Math.max(0.8, this.corridorWidth);
        const minRun = Math.max(2.2, cw * 1.8);
        const bandGuard = Math.max(0.08, Math.min(0.22, this.corridorGapClearance * 4));
        const axisMergeTol = Math.max(0.75, cw * 0.9);
        const fallbackStep = Math.max(1.4, Math.min(1.9, this.boxDepth * 0.75));

        const normalizeIntervals = (intervals, lower, upper) => {
            const raw = (intervals || [])
                .map((i) => [Math.max(lower, Math.min(upper, i[0])), Math.max(lower, Math.min(upper, i[1]))])
                .map(([a, z]) => [Math.min(a, z), Math.max(a, z)])
                .filter(([a, z]) => Number.isFinite(a) && Number.isFinite(z) && z > a)
                .sort((a, z) => a[0] - z[0]);
            const merged = [];
            for (const interval of raw) {
                if (!merged.length || interval[0] > merged[merged.length - 1][1] + 1e-6) {
        ...
        const subtractIntervals = (base, blocked) => {
            const merged = normalizeIntervals(blocked, base[0], base[1]);
            const out = [];
            let cursor = base[0];
        ...
        const dedupeAxis = (values) => {
            const sorted = (values || [])
                .filter(Number.isFinite)
                .sort((a, z) => a - z);
            const out = [];
            for (const v of sorted) {
                if (!out.length || Math.abs(v - out[out.length - 1]) > axisMergeTol) out.push(v);
            }
            return out;
        };

        const overlapLen1D = (a0, a1, b0, b1) => Math.max(0, Math.min(a1, b1) - Math.max(a0, b0));
        const facingSideTol = Math.max(0.30, cw * 0.55);
        const minFacingOverlap = Math.max(0.40, cw * 0.35);
        const hasFacingSupport = (rect) => {
            if (!rect) return false;
            const isHorizontal = rect.direction === 'horizontal' || rect.width >= rect.height;
            if (isHorizontal) {
                let hasTop = false;
                let hasBottom = false;
                const bandY0 = rect.y;
                const bandY1 = rect.y + rect.height;
                for (const u of unitRects) {
                    const ox = overlapLen1D(rect.x, rect.x + rect.width, u.x, u.x + u.width);
                    if (ox < minFacingOverlap) continue;
                    const topGap = Math.abs((u.y + u.height) - bandY0);
                    const bottomGap = Math.abs(u.y - bandY1);
                    if (topGap <= facingSideTol) hasTop = true;
                    if (bottomGap <= facingSideTol) hasBottom = true;
            ...
            let hasRight = false;
            const bandX0 = rect.x;
            const bandX1 = rect.x + rect.width;
            for (const u of unitRects) {
                const oy = overlapLen1D(rect.y, rect.y + rect.height, u.y, u.y + u.height);
                if (oy < minFacingOverlap) continue;
                const leftGap = Math.abs((u.x + u.width) - bandX0);
                const rightGap = Math.abs(u.x - bandX1);
                if (leftGap <= facingSideTol) hasLeft = true;
                if (rightGap <= facingSideTol) hasRight = true;
                if (hasLeft && hasRight) return true;
            }
            return false;
        };

        const buildHorizontalCandidates = () => {
            const yEdges = [];
            for (const u of unitRects) {
                yEdges.push(u.y, u.y + u.height);
            }
            yEdges.sort((a, z) => a - z);

            const centers = [];
            for (let i = 0; i < yEdges.length - 1; i++) {
                const gap = yEdges[i + 1] - yEdges[i];
                if (gap < cw * 0.90) continue;
                const center = (yEdges[i + 1] + yEdges[i]) / 2;
                if (center - cw / 2 < minY || center + cw / 2 > maxY) continue;
                centers.push(center);
            }
            for (let y = minY + cw / 2; y <= maxY - cw / 2; y += fallbackStep) centers.push(y);
            return dedupeAxis(centers);
        };

        const buildVerticalCandidates = () => {
            const xEdges = [];
            for (const u of unitRects) {
                xEdges.push(u.x, u.x + u.width);
            }
            xEdges.sort((a, z) => a - z);

            const centers = [];
            for (let i = 0; i < xEdges.length - 1; i++) {
                const gap = xEdges[i + 1] - xEdges[i];
                if (gap < cw * 0.90) continue;
                const center = (xEdges[i + 1] + xEdges[i]) / 2;
                if (center - cw / 2 < minX || center + cw / 2 > maxX) continue;
                centers.push(center);
            }
            for (let x = minX + cw / 2; x <= maxX - cw / 2; x += fallbackStep) centers.push(x);
            return dedupeAxis(centers);
        };

        const existing = (Array.isArray(existingCorridors) ? existingCorridors : [])
            .map((c) => this._normalizeCorridorRect(c))
            .filter(Boolean);
        const generated = [];
        const conflictsExisting = (candidate) => {
            return existing.some((c) => this._corridorsConflict(candidate, c) || this._corridorOverlapsCorridor(candidate, c));
        };
        const conflictsGenerated = (candidate) => {
            return generated.some((c) => this._corridorsConflict(candidate, c) || this._corridorOverlapsCorridor(candidate, c));
        };

        const horizontalCenters = buildHorizontalCandidates();
        for (const cy of horizontalCenters) {
            const y = cy - cw / 2;
            const blocked = [];
            for (const u of unitRects) {
                const overlapsY = (y + cw) > (u.y + bandGuard) && y < (u.y + u.height - bandGuard);
                if (!overlapsY) continue;
                blocked.push([u.x - bandGuard, u.x + u.width + bandGuard]);
            }
            const freeRuns = subtractIntervals([minX, maxX], blocked);
            for (const [sx0, sx1] of freeRuns) {
                const segW = sx1 - sx0;
                if (segW < minRun) continue;
                const rect = {
                    x: sx0,
                    y,
                    width: segW,
                    height: cw,
                    direction: 'horizontal',
                    type: 'supplemental_gap_grid'
                };
                if (this._boxHitsObstacle(rect.x, rect.y, rect.width, rect.height, { ignoreEntrances: true })) continue;
                if (!hasFacingSupport(rect)) continue;
                if (conflictsExisting(rect) || conflictsGenerated(rect)) continue;
                generated.push(rect);
            }
        }

        const verticalCenters = buildVerticalCandidates();
        for (const cx of verticalCenters) {
            const x = cx - cw / 2;
            const blocked = [];
            for (const u of unitRects) {
                const overlapsX = (x + cw) > (u.x + bandGuard) && x < (u.x + u.width - bandGuard);
                if (!overlapsX) continue;
                blocked.push([u.y - bandGuard, u.y + u.height + bandGuard]);
            }
            const freeRuns = subtractIntervals([minY, maxY], blocked);
            for (const [sy0, sy1] of freeRuns) {
                const segH = sy1 - sy0;
                if (segH < minRun) continue;
                const rect = {
                    x,
                    y: sy0,
                    width: cw,
        ...
        // Keep a broader supplemental set so hallway grid remains visible across the floor.
        // Prefer longer centerlines first, then cap total to avoid extreme clutter.
        generated.sort((a, b) => Math.max(b.width, b.height) - Math.max(a.width, a.height));
        const maxSupplement = Math.max(40, Math.min(140, Math.ceil(unitRects.length * 0.45)));
        return generated.slice(0, maxSupplement);
    }
    ...
    _fillResidualGaps(units, corridors, options = {}) {
        ...
        const corridorMeta = (Array.isArray(corridors) ? corridors : [])
            .map((c) => {
                const x = Number(c?.x);
                const y = Number(c?.y);
                ...
                return {
                    ...
                    width,
                    height,
                    horizontal: c?.direction === 'horizontal' || width >= height
                };
            })
            .filter(Boolean);
        const junctionPoints = [];
        for (let i = 0; i < corridorMeta.length; i++) {
            const a = corridorMeta[i];
            for (let j = i + 1; j < corridorMeta.length; j++) {
                const b = corridorMeta[j];
                if (!a || !b || a.horizontal === b.horizontal) continue;
                ...
                if (vx >= hx1 - 1e-6 && vx <= hx2 + 1e-6 && hy >= vy1 - 1e-6 && hy <= vy2 + 1e-6) {
                    junctionPoints.push({ x: vx, y: hy });
                }
            }
        }
        ...
        const isNearCorridorJunction = (rect) => {
            if (!junctionPoints.length) return false;
            const cx = rect.x + rect.width / 2;
            const cy = rect.y + rect.height / 2;
            const pinch = Math.max(0.60, this.corridorWidth * 0.85);
            for (const jp of junctionPoints) {
                if (Math.abs(cx - jp.x) <= pinch && Math.abs(cy - jp.y) <= pinch) return true;
            }
            return false;
        };
        ...
        const b = this.bounds;
        const cl = this.wallClearance;
        const edgeClearance = Math.max(0.02, cl * 0.2);
        ...
        let diagRegionReject = 0;
        let diagProbeOccupied = 0;
        let diagProbeCorridor = 0;
        let diagProbeJunction = 0;
        let diagRectOccupied = 0;
        let diagRectCorridor = 0;
        let diagRectJunction = 0;
        let diagRectInvalid = 0;
        ...
        for (let y = b.minY + edgeClearance; y + Math.min(depth, minWidth) <= b.maxY - edgeClearance + 1e-6; y += scanStep) {
            ...
            for (let x = b.minX + edgeClearance; x + minWidth <= b.maxX - edgeClearance + 1e-6; x += scanStep) {
                ...
                if (!this._isPointInValidGapFillRegion(sx, sy)) {
                    diagRegionReject++;
                    continue;
                }
                const probe = { x: sx, y: sy, width: minWidth, height: Math.min(depth, minWidth) };
                if (overlapWithOccupied(probe)) {
                    diagProbeOccupied++;
                    continue;
                }
                if (overlapWithCorridors(probe)) {
                    diagProbeCorridor++;
                    continue;
                }
                if (isNearCorridorJunction(probe)) {
                    diagProbeJunction++;
                    continue;
                }
                ...
                for (const size of sizeCatalog) {
                    ...
                    for (const rotated of [false, true]) {
                        ...
                        if (overlapWithOccupied(rect)) {
                            diagRectOccupied++;
                            continue;
                        }
                        if (overlapWithCorridors(rect)) {
                            diagRectCorridor++;
                            continue;
                        }
                        if (isNearCorridorJunction(rect)) {
                            diagRectJunction++;
                            continue;
                        }
                        if (!isGapFillValid(sx, sy, bw, bh)) {
                            diagRectInvalid++;
                            continue;
                        }
        ...
        console.log(
            `[CostoProLayout v4][Diag] Residual gap-fill scan: added=${added}, ` +
            `rejects(region=${diagRegionReject}, probeOccupied=${diagProbeOccupied}, probeCorridor=${diagProbeCorridor}, ` +
            `probeJunction=${diagProbeJunction}, rectOccupied=${diagRectOccupied}, rectCorridor=${diagRectCorridor}, ` +
            `rectJunction=${diagRectJunction}, invalid=${diagRectInvalid})`
        );
    ...
    generate(config = {}) {
        ...
        // Step 2b: Gap-filling pass — place boxes in empty space; corridors in occupied so corridor space is preserved
        {
            ...
            const b = this.bounds;
            const cl = this.wallClearance;
            const gapFillCl = Math.max(0.02, cl * 0.45);  // Tighter clearance to fill more gaps; corridor space kept via occupied
        ...
        // Step 2b-2: Second gap-fill pass — tighter clearance (0.02) and finer step (0.3) to fill remaining empty areas everywhere
        {
            const gapFill2Before = units.length;
            const bd = this.boxDepth;
            const b = this.bounds;
            const gapFillCl2 = 0.015;
        ...
        // Step 2b-4: Micro gap-fill — very fine step (0.2) and minimal clearance (0.01) to pack small boxes in remaining slivers
        {
        ...
        // Step 2b-5: Nano fill — step 0.15, 0.80m boxes only, minimal clearance to fill last slivers
        {
        ...
        const coverageBeforeSupplement = this._corridorCoverageNeedsSupplement(finalUnits, finalCorridors);
        if (coverageBeforeSupplement.needsSupplement) {
            const supplemental = this._buildUnitGapCorridorGrid(finalUnits, finalCorridors);
            if (supplemental.length > 0) {
                const supplemented = this._sanitizeFinalLayout(finalUnits, finalCorridors.concat(supplemental));
                finalUnits = supplemented.units;
                finalCorridors = this._linkDisconnectedCorridors(supplemented.corridors, finalUnits);
                console.log(
                    `[CostoProLayout v4] Supplemental corridors: +${supplemental.length} ` +
                    `(xRatio=${(coverageBeforeSupplement.xRatio || 0).toFixed(2)}, yRatio=${(coverageBeforeSupplement.yRatio || 0).toFixed(2)})`
                );
            } else {
                console.log(
                    `[CostoProLayout v4] Supplemental corridors: none ` +
                    `(xRatio=${(coverageBeforeSupplement.xRatio || 0).toFixed(2)}, yRatio=${(coverageBeforeSupplement.yRatio || 0).toFixed(2)})`
                );
            }
        }
    ...
    _planBayStrips(bay, startId, sizes, sizeIdx) {
        ...
        const minFillWidth = widthCatalog[widthCatalog.length - 1].width;
        const placementDiag = {
            rowTailMisses: 0,
            rowTailRecovered: 0,
            rowTailSamples: [],
            rowTailRecoveredSamples: []
        };

        const normalizeSize = (sizeLike) => {
            const width = Number(sizeLike?.width);
            if (!Number.isFinite(width) || width <= 0) return null;
            let type = typeof sizeLike?.type === 'string' ? sizeLike.type : null;
            if (!type) {
                if (width >= 3.0) type = 'XL';
                else if (width >= 2.0) type = 'L';
                else if (width >= 1.45) type = 'M';
                else type = 'S';
            }
            return {
                type,
                width,
                area: Number.isFinite(Number(sizeLike?.area)) ? Number(sizeLike.area) : width * bd
            };
        };

        const buildCandidates = (preferred) => {
            const ordered = [];
            const usedWidths = new Set();

            const preferredNorm = normalizeSize(preferred);
            if (preferredNorm) {
                ordered.push({ ...preferredNorm, source: 'preferred' });
                usedWidths.add(preferredNorm.width.toFixed(3));
            }

            for (const item of widthCatalog) {
                const key = Number(item.width).toFixed(3);
                if (usedWidths.has(key)) continue;
                ordered.push({
                    type: item.type,
                    width: item.width,
                    area: item.width * bd,
        ...
        const buildRectForWidth = (row, cursorValue, widthValue) => {
            if (isHorizontal) {
                const bx = this._snapToPlacementGrid(cursorValue);
                const by = this.rowAlignmentStep ? this._snapToRowAlignment(row.y) : row.y;
                return { bx, by, bw: widthValue, bh: bd };
            }

            const bx = this._snapToPlacementGrid(row.x);
            const by = this.rowAlignmentStep ? this._snapToRowAlignment(cursorValue) : cursorValue;
            return { bx, by, bw: bd, bh: widthValue };
        };

        const placeRow = (row, isSecondRow) => {
            let cursor = runStart;
            while (cursor + this.minGapLength <= runEnd) {
                const preferredSize = sizes[sizeIdx % sizes.length];
                const preferredWidth = Number(preferredSize?.width);
                const remaining = runEnd - cursor;

                let chosen = null;
                let fitButInvalid = false;
                let anyFitByLength = false;

                for (const candidate of buildCandidates(preferredSize)) {
                    const targetW = Number(candidate.width);
                    if (!Number.isFinite(targetW) || targetW <= 0) continue;
                    if (cursor + targetW > runEnd + 1e-6) continue;

                    anyFitByLength = true;
                    const trial = buildRectForWidth(row, cursor, targetW);
                    if (!this._isBoxValid(trial.bx, trial.by, trial.bw, trial.bh)) {
                        fitButInvalid = true;
                        continue;
                    }
                ...
                if (!chosen) {
                    if (anyFitByLength && fitButInvalid) {
                        // Local obstruction at this cursor: advance slightly and keep scanning,
                        // otherwise we can prematurely stop and leave usable tail space unfilled.
                        cursor += Math.max(this.placementGridStep, 0.20);
                        continue;
                    }

                    const remaining = runEnd - cursor;
                    const requested = preferredWidth;
                    const couldFitSmaller = Number.isFinite(remaining) && remaining >= minFillWidth - 1e-6;
                    if (Number.isFinite(requested) && requested > remaining + 1e-6 && couldFitSmaller) {
                        placementDiag.rowTailMisses += 1;
                        if (placementDiag.rowTailSamples.length < 6) {
                            placementDiag.rowTailSamples.push({
                                bayId: bay.id,
                                row: row.side,
                                remaining: Number(remaining.toFixed(3)),
                                requested: Number(requested.toFixed(3)),
                                minFillWidth,
                                cursor: Number(cursor.toFixed(3)),
                                runEnd: Number(runEnd.toFixed(3))
                            });
                        }
                    }
                    break;
                ...
                if (
                    ...
                    preferredWidth > remaining + 1e-6
                ) {
                    placementDiag.rowTailRecovered += 1;
                    if (placementDiag.rowTailRecoveredSamples.length < 6) {
                        placementDiag.rowTailRecoveredSamples.push({
                            bayId: bay.id,
                            row: row.side,
                            remaining: Number(remaining.toFixed(3)),
                            requested: Number(preferredWidth.toFixed(3)),
                            fallbackWidth: Number(chosen.width.toFixed(3)),
                            fallbackType: chosen.type,
                            cursor: Number(cursor.toFixed(3)),
                            runEnd: Number(runEnd.toFixed(3))
                        });
                    }
                ...
                const unit = this._makeUnit(
                    ...
                    chosen.bw,
                    chosen.bh,
                    { type: chosen.type, width: chosen.width, area: chosen.area },
                    row.side,
                    !isHorizontal
                );
                units.push(unit);

                cursor += chosen.width + sp;
            }
        };
        ...
        if (placementDiag.rowTailMisses > 0) {
            console.log(
                `[CostoProLayout v4][Diag] Bay ${bay.id} row-tail misses=${placementDiag.rowTailMisses} ` +
                `(requested width did not fit while smaller widths could)`,
                placementDiag.rowTailSamples
            );
        }

        if (placementDiag.rowTailRecovered > 0) {
            console.log(
                `[CostoProLayout v4][Diag] Bay ${bay.id} row-tail recovered=${placementDiag.rowTailRecovered} ` +
                `(used fallback smaller width when requested size did not fit)`,
                placementDiag.rowTailRecoveredSamples
            );
        }
