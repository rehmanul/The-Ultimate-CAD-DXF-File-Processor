...
function generateEnvelopeSubdivision(bounds, floorPlan, unitMix, distribution, options = {}) {
    ...
    let unitScale = 1.0;
    if (maxDim > 2000) unitScale = 1000.0; // Millimeters
    else if (maxDim > 200) unitScale = 100.0; // Centimeters

    // Define parameters with SCALING applied immediately
    // This allows the rest of the function to use 'margin', 'wallClearance' etc without change
    ...
    const cellSpacing = 0.1 * unitScale; // Gap between cells
    const wallClearance = 0.4 * unitScale; // Distance to keep from walls
    ...
    const fillerCells = [];
    const gapMinWidth = 0.5;
    const gapMinHeight = 0.5;
    ...
    function overlapsExisting(x, y, w, h) {
        for (const occ of occupied) {
            if (x < occ.maxX && x + w > occ.minX &&
                ...
                return true;
                if (x < occ.maxX - margin && x + w > occ.minX + margin &&
                    y < occ.maxY - margin && y + h > occ.minY + margin) {
                    return true;
                }
            }
            return false;
                ...
                if (gapWidth >= gapMinWidth && gapHeight >= gapMinHeight) {
                    ...
                    if (!isCellBlocked(gapX, gapY, gapWidth, gapHeight) &&
                        ...
                        fillerCells.push({
                            ...
                            area: area, type: type, color: '#a7f3d0', // Default light green for fillers
        ...
        if (gridWidth * gridHeight < 100000) {
            ...
            const maxFillers = 50; // Limit to prevent infinite loop
            ...
            for (let attempt = 0; attempt < maxFillers; attempt++) {
                ...
                // Sample grid with strides to find openings
                for (let gy = 1; gy < gridHeight - 1; gy += 2) {
                    for (let gx = 1; gx < gridWidth - 1; gx += 2) {
                        if (grid[gy * gridWidth + gx] === 0) {
                            // Found a free seed, try to expand a rectangle here
                            // Expand Right
                            let rw = 1;
                            while (gx + rw < gridWidth && grid[gy * gridWidth + (gx + rw)] === 0 && (gx + rw) * gridSize < 10.0) { // Limit max width
                                rw++;
                            }

                            // Expand Down (for this width)
                            let rh = 1;
                            let valid = true;
                            while (valid && gy + rh < gridHeight && (gy + rh) * gridSize < 10.0) {
                                for (let k = 0; k < rw; k++) {
                            ...
                            // Favor roughly square or standard sized spots
                            if (realW >= 0.8 && realH >= 0.8) {
                                const score = realW * realH;
                                if (score > maxScore) {
                                    maxScore = score;
                                    bestRect = { x: gx, y: gy, w: rw, h: rh, realW, realH };
                                }
                            }
                        }
                    }
                ...
                if (bestRect) {
                    ...
                    if (!isCellBlocked(finalX, finalY, bestRect.realW, bestRect.realH) &&
                        ...
                        fillerCells.push({
                            ...
                            area: area, type: type, color: '#86efac', // Distinguishable filler color
