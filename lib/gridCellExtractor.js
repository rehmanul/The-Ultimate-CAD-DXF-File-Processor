...
function generateEnvelopeSubdivision(bounds, floorPlan, unitMix, distribution, options = {}) {
    ...
    // Aggressive space filling settings
    const corridorWidth = (Number.isFinite(options.corridorWidth) ? options.corridorWidth : 0.9) * unitScale;  // Reduced from 1.2
    const margin = (Number.isFinite(options.margin) ? options.margin : 0.25) * unitScale;  // Reduced from 0.5
    const cellSpacing = 0.03 * unitScale;  // Reduced from 0.1
    const wallClearance = 0.12 * unitScale;  // Reduced from 0.2
    ...
    const baseRowHeight = Math.max(1.2, Math.min(2.5, Math.sqrt((avgArea / areaScale) * 1.3)));  // Reduced heights
    const rowHeight = baseRowHeight * unitScale;

    const rowGap = corridorWidth * 0.08; // Minimal gap between rows (reduced from 0.15)
    const mainCorridorGap = corridorWidth * 0.6; // Main corridor every 4 rows (reduced from 1.0)
    ...
    let rowIndex = 0;
    let skipCounter = 0;
    const maxSkipsPerRow = 200; // Increased for better space exploration
    ...
    const fillerCells = [];
    const gapMinWidth = 0.2 * unitScale;  // Reduced from 0.3 for more aggressive filling
    ...
    if (gridWidth * gridHeight < 100000) {
        ...
        const maxFillers = 500; // Increased from 200 for maximum coverage
...
/**
 * PHASE 1: Professional Row Alignment Engine
 * Aligns ilots into uniform rows like COSTO reference drawings
 * 
 * @param {Array} ilots - Array of ilots to align
 * @param {Object} bounds - Floor plan bounds { minX, maxX, minY, maxY }
 * @param {Object} options - Configuration options
 * @returns {Array} Aligned ilots with uniform row heights
 */
function alignIlotsToRows(ilots, bounds, options = {}) {
    const {
        gridSnap = 0.5,           // Snap to 0.5m grid
        corridorWidth = 1.2,       // Corridor width between rows
        minRowHeight = 1.2,        // Minimum row height
        maxRowHeight = 4.0,        // Maximum row height
        rowTolerance = 0.3,        // Y-band grouping tolerance
        fillGaps = true,           // Fill gaps with new ilots
        respectExclusionZones = true
    } = options;

    if (!ilots || ilots.length === 0) {
        console.log('[RowAlign] No ilots to align');
        return [];
    }

    console.log(`[RowAlign] Aligning ${ilots.length} ilots to rows...`);

    // Step 1: Group ilots by Y-coordinate bands (rows)
    const rows = new Map(); // rowY -> ilots in that row

    ilots.forEach(ilot => {
        const centerY = ilot.y + (ilot.height / 2);
        const snappedY = Math.round(centerY / rowTolerance) * rowTolerance;

        if (!rows.has(snappedY)) {
            rows.set(snappedY, []);
        }
        rows.get(snappedY).push({ ...ilot });
    });

    console.log(`[RowAlign] Found ${rows.size} distinct rows`);

    // Step 2: Sort rows by Y (bottom to top for drafting convention)
    const sortedRowYs = Array.from(rows.keys()).sort((a, b) => a - b);

    // Step 3: Process each row - uniform height, left-to-right fill
    const alignedIlots = [];
    let currentY = bounds ? bounds.minY + corridorWidth : sortedRowYs[0];
    let ilotIndex = 0;

    sortedRowYs.forEach((rowY, rowIndex) => {
        const rowIlots = rows.get(rowY);

        // Calculate uniform height for this row (median height)
        const heights = rowIlots.map(i => i.height).filter(h => h > 0);
        heights.sort((a, b) => a - b);
        let uniformHeight = heights.length > 0
            ? heights[Math.floor(heights.length / 2)]
            : minRowHeight;

        // Clamp to min/max
        uniformHeight = Math.max(minRowHeight, Math.min(maxRowHeight, uniformHeight));
        uniformHeight = Math.round(uniformHeight / gridSnap) * gridSnap;

        // Sort ilots in row by X (left to right)
        rowIlots.sort((a, b) => a.x - b.x);

        // Determine row Y position (add corridor gap between rows)
        const rowStartY = rowIndex === 0
            ? currentY
            : currentY + (rowIndex > 0 ? corridorWidth : 0);

        // Process each ilot in the row
        let currentX = bounds ? bounds.minX : rowIlots[0]?.x || 0;

        rowIlots.forEach((ilot, colIndex) => {
            // Snap X position to grid
            const snappedX = Math.round(currentX / gridSnap) * gridSnap;

            // Snap width to grid
            let snappedWidth = Math.round(ilot.width / gridSnap) * gridSnap;
            snappedWidth = Math.max(gridSnap, snappedWidth); // Minimum one grid cell
            ...
            const alignedIlot = {
                ...
                area: snappedWidth * uniformHeight,
                row: rowIndex,
                col: colIndex,
                label: `${(snappedWidth * uniformHeight).toFixed(1)}m²`,
                dimensions: `${snappedWidth.toFixed(1)} x ${uniformHeight.toFixed(1)}`
            };

            alignedIlots.push(alignedIlot);
            currentX = snappedX + snappedWidth; // Move to next X position (no gap within row)
            ilotIndex++;
        });

        // Fill remaining space in row if fillGaps is enabled
        if (fillGaps && bounds) {
            const lastIlot = alignedIlots[alignedIlots.length - 1];
            const rowEndX = lastIlot ? lastIlot.x + lastIlot.width : currentX;
            const remainingWidth = bounds.maxX - rowEndX;

            if (remainingWidth >= gridSnap * 2) { // At least 2 grid cells worth
                // Calculate how many additional ilots to fill
                const avgWidth = rowIlots.reduce((sum, i) => sum + i.width, 0) / rowIlots.length;
                const snappedAvgWidth = Math.round(avgWidth / gridSnap) * gridSnap;
                const fillCount = Math.floor(remainingWidth / snappedAvgWidth);

                for (let f = 0; f < fillCount; f++) {
                    const fillX = rowEndX + (f * snappedAvgWidth);
                    if (fillX + snappedAvgWidth <= bounds.maxX) {
                        alignedIlots.push({
                            id: `BOX_${ilotIndex + 1}`,
                            x: fillX,
                            y: rowStartY,
                            width: snappedAvgWidth,
                            height: uniformHeight,
                            area: snappedAvgWidth * uniformHeight,
                            row: rowIndex,
                            col: rowIlots.length + f,
                            type: 'filler',
                            label: `${(snappedAvgWidth * uniformHeight).toFixed(1)}m²`,
                            dimensions: `${snappedAvgWidth.toFixed(1)} x ${uniformHeight.toFixed(1)}`
                        });
                        ilotIndex++;
                    }
                }
        ...
        currentY = rowStartY + uniformHeight;
    });

    // Step 4: Final sort and re-index
    alignedIlots.sort((a, b) => (a.row - b.row) || (a.col - b.col));
    alignedIlots.forEach((ilot, idx) => {
        ilot.id = `BOX_${idx + 1}`;
    });

    console.log(`[RowAlign] Aligned ${alignedIlots.length} ilots into ${rows.size} rows`);

    return alignedIlots;
}

/**
 * Generate professional ilot layout from distribution
 * Creates row-aligned ilots respecting unit mix distribution
 * 
 * @param {Object} floorPlan - Parsed floor plan
 * @param {Object} distribution - Unit mix distribution (percentages by type)
 * @param {Array} unitMix - Unit mix typologies
 * @param {Object} options - Configuration options
 * @returns {Array} Professional layout ilots
 */
function generateProfessionalLayout(floorPlan, distribution, unitMix, options = {}) {
    const bounds = floorPlan?.bounds;
    if (!bounds) {
        console.error('[ProLayout] No bounds in floor plan');
        return [];
    }
    ...
    const {
        corridorWidth = 1.2,
        gridSnap = 0.5,
        targetDensity = 0.7,  // 70% usable, 30% corridors
        exclusionZones = []
    } = options;

    console.log('[ProLayout] Generating professional COSTO-style layout...');

    // Calculate total usable area
    const totalWidth = bounds.maxX - bounds.minX;
    const totalHeight = bounds.maxY - bounds.minY;
    const totalArea = totalWidth * totalHeight;
    const usableArea = totalArea * targetDensity;

    // Calculate row parameters
    const avgTypoArea = unitMix && unitMix.length > 0
        ? unitMix.reduce((sum, t) => sum + (t.targetArea || 0), 0) / unitMix.length
        : 2.5; // Default 2.5m²

    // Standard row height (from unit mix or default)
    const standardHeight = Math.sqrt(avgTypoArea * 0.8); // 0.8 aspect ratio
    const rowHeight = Math.round(standardHeight / gridSnap) * gridSnap;

    // Calculate number of rows
    const numRows = Math.floor((totalHeight - corridorWidth) / (rowHeight + corridorWidth));

    console.log(`[ProLayout] Floor: ${totalWidth.toFixed(1)}m x ${totalHeight.toFixed(1)}m = ${totalArea.toFixed(1)}m²`);
    console.log(`[ProLayout] Planning ${numRows} rows, row height: ${rowHeight.toFixed(2)}m`);

    const ilots = [];
    let ilotId = 0;

    // Generate ilots for each row based on distribution
    for (let rowIdx = 0; rowIdx < numRows; rowIdx++) {
        const rowY = bounds.minY + corridorWidth + rowIdx * (rowHeight + corridorWidth);
        let currentX = bounds.minX;
        let colIdx = 0;

        // Fill row with ilots based on unit mix distribution
        while (currentX < bounds.maxX - gridSnap) {
            // Select type from distribution
            const selectedType = selectTypeFromDistribution(distribution, unitMix, ilotId);
            const targetArea = selectedType?.targetArea || avgTypoArea;

            // Calculate width from target area and row height
            let ilotWidth = targetArea / rowHeight;
            ilotWidth = Math.max(gridSnap, Math.round(ilotWidth / gridSnap) * gridSnap);

            // Ensure ilot fits in remaining row space
            const remainingWidth = bounds.maxX - currentX;
            if (ilotWidth > remainingWidth) {
                ilotWidth = Math.floor(remainingWidth / gridSnap) * gridSnap;
            }

            if (ilotWidth < gridSnap) break;

            const actualArea = ilotWidth * rowHeight;
            ...
            ilots.push({
                ...
                height: rowHeight,
                area: actualArea,
                type: selectedType?.name || 'M',
                row: rowIdx,
                col: colIdx,
                label: `${actualArea.toFixed(1)}m²`,
                dimensions: `${ilotWidth.toFixed(1)} x ${rowHeight.toFixed(1)}`
            });
    ...
    console.log(`[ProLayout] Generated ${ilots.length} professional ilots in ${numRows} rows`);
    return ilots;
}

/**
 * Select a type from distribution based on cumulative probability
 */
function selectTypeFromDistribution(distribution, unitMix, seed) {
    if (!unitMix || unitMix.length === 0) return null;

    // Build cumulative distribution
    const total = Object.values(distribution || {}).reduce((s, v) => s + (parseFloat(v) || 0), 0) || unitMix.length;
    let cumulative = 0;
    const cdf = [];

    unitMix.forEach((typo, idx) => {
        const weight = distribution?.[typo.name] || distribution?.[idx] || (100 / unitMix.length);
        cumulative += parseFloat(weight) / total;
        cdf.push({ cumulative, typo });
    });

    // Use seed for deterministic selection
    const rand = ((seed * 1.618033988749) % 1); // Golden ratio for good distribution
    ...
    return unitMix[unitMix.length - 1];
}

module.exports = {
    extractGridCells,
    generateCorridors,
    generateExclusionZones,
    renderExclusionZonesData,
    alignIlotsToRows,
