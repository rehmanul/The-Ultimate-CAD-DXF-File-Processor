...
function generateEnvelopeSubdivision(bounds, floorPlan, unitMix, distribution, options = {}) {
    ...
    const wallClearance = 0.8; // Distance to keep from walls (increased for better avoidance)
    ...
    const rowGap = corridorWidth * 0.3; // Small gap between rows
    const mainCorridorGap = corridorWidth * 2.0; // PROMINENT main corridor every 4 rows
    const secondaryCorridorGap = corridorWidth * 1.2; // Secondary corridor every 2 rows
    ...
    function isCellBlocked(x, y, w, h) {
        ...
        for (const wall of wallSegs) {
            ...
            for (const [px, py] of corners) {
                ...
                if (cornerDist < wallClearance * 0.7) return true;
            }
            ...
            // Additional check: sample points along each edge
            for (const [[ex1, ey1], [ex2, ey2]] of edges) {
                for (let t = 0.25; t <= 0.75; t += 0.25) {
                    const px = ex1 + t * (ex2 - ex1);
                    const py = ey1 + t * (ey2 - ey1);
                    const edgeDist = distToSegment(px, py, wall.x1, wall.y1, wall.x2, wall.y2);
                    if (edgeDist < wallClearance * 0.5) return true;
                }
            }
        }
        ...
        for (const fb of forbiddenBounds) {
            if (cx >= fb.minX - 0.5 && cx <= fb.maxX + 0.5 &&
                cy >= fb.minY - 0.5 && cy <= fb.maxY + 0.5) {
                return true;
            }
        }
        ...
        for (const ent of entranceBounds) {
            if (cx >= ent.minX - 1.0 && cx <= ent.maxX + 1.0 &&
                cy >= ent.minY - 1.0 && cy <= ent.maxY + 1.0) {
                return true;
            }
        }
    ...
    while (currentY + rowHeight <= maxY) {
        ...
        // Add gap (main corridor, secondary corridor, or small row gap)
        if (rowIndex % 4 === 0) {
            currentY += mainCorridorGap; // PROMINENT main corridor
        } else if (rowIndex % 2 === 0) {
            currentY += secondaryCorridorGap; // Secondary corridor
        } else {
            currentY += rowGap; // Small gap
        }
    }
