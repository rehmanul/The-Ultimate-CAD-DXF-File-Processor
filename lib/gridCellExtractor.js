 ...
 * Creates L-shaped paths connecting entrances through the floor plan
 */
function generateCorridors(floorPlan, cells, options = {}) {
    const { corridorWidth = 1.5 } = options;
    const corridors = [];
    const entrances = floorPlan?.entrances || [];

    if (entrances.length < 2) {
        console.log('[Corridors] Less than 2 entrances, skipping corridor generation');
        return corridors;
    }

    // Get entrance centers
    const entranceCenters = entrances.map(ent => {
    ...
    // Connect entrances with L-shaped corridors
    for (let i = 0; i < entranceCenters.length - 1; i++) {
        const start = entranceCenters[i];
        const end = entranceCenters[i + 1];

        // Create L-shaped path: horizontal then vertical
        const midX = (start.x + end.x) / 2;

        // Horizontal segment from start to midX
        const hCorridor = {
            id: `CORRIDOR_H_${i}`,
            type: 'horizontal',
            x: Math.min(start.x, midX) - corridorWidth / 2,
            y: start.y - corridorWidth / 2,
            width: Math.abs(midX - start.x) + corridorWidth,
            height: corridorWidth,
            isMainCorridor: true
        };

        // Vertical segment from start.y to end.y at midX
        const vCorridor = {
            id: `CORRIDOR_V_${i}`,
            type: 'vertical',
            x: midX - corridorWidth / 2,
            y: Math.min(start.y, end.y) - corridorWidth / 2,
            width: corridorWidth,
            height: Math.abs(end.y - start.y) + corridorWidth,
            isMainCorridor: true
        };

        // Horizontal segment from midX to end
        const hCorridor2 = {
            id: `CORRIDOR_H2_${i}`,
            type: 'horizontal',
            x: Math.min(midX, end.x) - corridorWidth / 2,
            y: end.y - corridorWidth / 2,
            width: Math.abs(end.x - midX) + corridorWidth,
            height: corridorWidth,
            isMainCorridor: true
        };

        corridors.push(hCorridor, vCorridor, hCorridor2);
    }

    console.log(`[Corridors] Generated ${corridors.length} corridor segments connecting ${entranceCenters.length} entrances`);
    return corridors;
}
...
module.exports = { extractGridCells, generateCorridors };
