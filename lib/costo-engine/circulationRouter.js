...
class CirculationRouter {
    ...
    /**
     * Detect disconnected components in the circulation network.
     * Returns connectivity analysis including disconnected components and unreachable îlots.
     *
     * @param {Array} corridors - Array of corridor rectangles
     * @param {Array} îlots - Array of îlot/unit objects with x, y, width, height
     * @param {Array} entrances - Array of entrance points
     * @returns {Object} Connectivity analysis with:
     *   - isFullyConnected: boolean indicating if network is fully connected
     *   - disconnectedComponents: array of component objects with nodeKeys and îlots
     *   - unreachableÎlots: array of îlots with no corridor access
     *   - unreachableFromEntrances: array of corridor segments not reachable from any entrance
     */
    detectConnectivity(corridors, îlots, entrances) {
        if (!Array.isArray(corridors) || corridors.length === 0) {
            return {
                isFullyConnected: false,
                disconnectedComponents: [],
                unreachableÎlots: îlots || [],
                unreachableFromEntrances: [],
                totalComponents: 0
            };
        }

        // Build the circulation graph from corridor segments
        const axisSegments = this._buildCenterlineSegments(corridors);
        ...
        const { nodes, edges } = this._buildCenterlineGraph(axisSegments);

        // Detect disconnected components using DFS
        const { components, componentByNode } = this._buildConnectedComponentsFromCenterlineGraph(nodes);

        // Build component objects with node information
        const componentObjects = components.map((nodeKeys, index) => {
            const componentNodes = nodeKeys.map(key => {
                const node = nodes.get(key);
                return node ? { x: node.x, y: node.y, key: node.key } : null;
            }).filter(Boolean);
            ...
            return {
                ...
                îlots: []  // Will be populated below
            };
        });

        // Identify which îlots belong to which components (or are isolated)
        const unreachableÎlots = [];
        if (Array.isArray(îlots)) {
            for (const îlot of îlots) {
                const îlotConnected = this._findÎlotComponent(îlot, componentObjects, corridors);
                if (îlotConnected !== null) {
                    componentObjects[îlotConnected].îlots.push(îlot);
                } else {
                    unreachableÎlots.push(îlot);
                }
            }
        }

        // Identify nodes unreachable from entrances
        const unreachableFromEntrances = this._findUnreachableFromEntrances(
            nodes,
        ...
        const isFullyConnected = components.length <= 1 &&
                                 ...
                                 unreachableFromEntrances.length === 0;

        console.log(
            `[CirculationRouter] Connectivity: ${components.length} component(s), ` +
            `${unreachableÎlots.length} unreachable îlot(s), ` +
            `${unreachableFromEntrances.length} node(s) unreachable from entrances`
        );

        return {
            isFullyConnected,
            disconnectedComponents: componentObjects,
            unreachableÎlots,
            unreachableFromEntrances,
    ...
    /**
     * Find which component (if any) an îlot is connected to.
     * An îlot is considered connected if it overlaps or is adjacent to a corridor.
     *
     * @param {Object} îlot - Îlot object with x, y, width, height
     * @param {Array} componentObjects - Array of component objects
     * @param {Array} corridors - Array of corridor rectangles
     * @returns {number|null} Component index or null if isolated
     */
    _findÎlotComponent(îlot, componentObjects, corridors) {
        const x = Number(îlot.x);
        const y = Number(îlot.y);
        const w = Number(îlot.width);
        const h = Number(îlot.height);

        if (![x, y, w, h].every(Number.isFinite) || w <= 0 || h <= 0) {
            return null;
        }

        const adjacencyTolerance = 0.5; // îlot is connected if within this distance of corridor

        // Check each corridor to see if îlot is adjacent/overlapping
        for (const corridor of corridors) {
            const cx = Number(corridor.x);
            const cy = Number(corridor.y);
            const cw = Number(corridor.width);
            const ch = Number(corridor.height);

            if (![cx, cy, cw, ch].every(Number.isFinite) || cw <= 0 || ch <= 0) {
                continue;
            }

            // Check if îlot and corridor are adjacent or overlapping
            const îlotRight = x + w;
            const îlotBottom = y + h;
            const corridorRight = cx + cw;
            const corridorBottom = cy + ch;

            const horizontalOverlap = !(îlotRight + adjacencyTolerance < cx || x > corridorRight + adjacencyTolerance);
            const verticalOverlap = !(îlotBottom + adjacencyTolerance < cy || y > corridorBottom + adjacencyTolerance);

            if (horizontalOverlap && verticalOverlap) {
                // Îlot is adjacent to this corridor, find which component this corridor belongs to
                const corridorCenter = {
                    x: cx + cw / 2,
                    y: cy + ch / 2
                };

                // Find the component that contains a node near this corridor
                for (let i = 0; i < componentObjects.length; i++) {
                    const component = componentObjects[i];
                    for (const node of component.nodes) {
                        const dist = Math.hypot(node.x - corridorCenter.x, node.y - corridorCenter.y);
    ...
    /**
     * Find nodes that are not reachable from any entrance.
     * Uses BFS/DFS from entrance nodes to identify unreachable regions.
     *
     * @param {Map} nodes - Map of graph nodes
     * @param {Array} entrances - Array of entrance points
     * @returns {Array} Array of node keys that are unreachable from entrances
     */
    _findUnreachableFromEntrances(nodes, entrances) {
        if (!nodes || nodes.size === 0) {
            return [];
        }

        const entrancePoints = Array.isArray(entrances) && entrances.length > 0
            ? this._getEntrancePointsFromArray(entrances)
            : this._getEntrancePoints();

        if (entrancePoints.length === 0) {
            // No entrances means all nodes are technically unreachable
            return Array.from(nodes.keys());
        }

        // Find entrance nodes (nearest nodes to entrance points)
        const entranceNodes = new Set();
        for (const entPoint of entrancePoints) {
            const nearest = this._findNearestNode(nodes, entPoint);
            if (nearest) {
                entranceNodes.add(nearest.key);
            }
        }

        if (entranceNodes.size === 0) {
            // No entrance nodes found, all nodes are unreachable
            return Array.from(nodes.keys());
        }

        // Perform BFS from all entrance nodes to find reachable nodes
        ...
        while (queue.length > 0) {
            ...
            // Handle both node types: 'edges' (Set) from centerline graph and 'neighbors' (array) from segment graph
            const connections = node.edges || node.neighbors || [];
            
            if (node.edges) {
                // Centerline graph nodes have 'edges' as a Set of keys
                for (const edgeKey of connections) {
                    if (!reachable.has(edgeKey)) {
                        reachable.add(edgeKey);
                        queue.push(edgeKey);
                    }
                }
            } else if (node.neighbors) {
                // Segment graph nodes have 'neighbors' as an array of objects with 'toKey'
                for (const neighbor of connections) {
                    if (!reachable.has(neighbor.toKey)) {
                        reachable.add(neighbor.toKey);
                        queue.push(neighbor.toKey);
                    }
                }
            }
        }
    ...
    _buildCenterlineGraph(segments) {
        ...
        this._addAlignmentBridges(nodes, edges);
        return { nodes, edges, blockedEdges };
    }

    /**
     * Connect left-right (or top-bottom) corridor endpoints that are aligned but not
     * yet connected, so pathways appear continuous (e.g. two corridors either side of a block).
     */
    _addAlignmentBridges(nodes, edges) {
        const nodeList = Array.from(nodes.values());
        const edgeKeys = new Set();
        for (const e of edges) {
            edgeKeys.add(this._edgeKey(this._pointKey(e.p1), this._pointKey(e.p2)));
        }
        const maxBridgeGap = Math.max(4, (this.bounds?.maxX - this.bounds?.minX || 50) * 0.15);
        const alignTol = 0.25;
        ...
        for (let i = 0; i < nodeList.length; i++) {
            ...
            for (let j = i + 1; j < nodeList.length; j++) {
                ...
                const key = this._edgeKey(a.key, b.key);
                if (edgeKeys.has(key)) continue;

                if (dy <= alignTol && dx >= 0.3 && dx <= maxBridgeGap) {
                    if (!this._segmentCrossesWall(a, b)) {
                        edges.push({ axis: 'H', p1: a, p2: b, length: dx });
                        a.edges.add(b.key);
                        b.edges.add(a.key);
                        edgeKeys.add(key);
                    }
                } else if (dx <= alignTol && dy >= 0.3 && dy <= maxBridgeGap) {
                    if (!this._segmentCrossesWall(a, b)) {
                        edges.push({ axis: 'V', p1: a, p2: b, length: dy });
                        a.edges.add(b.key);
                        b.edges.add(a.key);
                        edgeKeys.add(key);
                    }
    ...
    /**
     * Build connected components from centerline graph nodes.
     * This version works with nodes that have 'edges' (Set) instead of 'neighbors' (array).
     *
     * @param {Map} nodes - Map of nodes from _buildCenterlineGraph (with 'edges' property)
     * @returns {Object} Object with components array and componentByNode map
     */
    _buildConnectedComponentsFromCenterlineGraph(nodes) {
        const components = [];
        const componentByNode = new Map();
        const visited = new Set();
        ...
        for (const key of nodes.keys()) {
            ...
            while (stack.length > 0) {
                ...
                // Nodes from _buildCenterlineGraph have 'edges' (Set), not 'neighbors' (array)
                if (node.edges) {
