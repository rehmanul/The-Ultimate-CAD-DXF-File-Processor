...
class ProfessionalCADProcessor {
    ...
    async _processParsedDXF(dxf) {
        ...
        if (this.rooms.length === 0) {
            console.log('[CAD Processor] No rooms from DXF, using room detector');
            try {
                const roomDetector = require('./roomDetector');
                // Detect rooms using the (potentially gap-closed) walls
                const detectedRooms = roomDetector.detectRooms(this.walls, this.entrances, this.forbiddenZones, this.bounds);
                if (detectedRooms.length > 0) {
                    console.log(`[CAD Processor] Room detector found ${detectedRooms.length} rooms`);
                    this.rooms = detectedRooms;
                }
            } catch (e) {
     ...
     */
    _closeWallGaps() {
        const maxGap = 2.0; // Connect endpoints within 2 meters
        const endpoints = [];
        ...
        // Find endpoint connections (each endpoint should connect to exactly one other)
        ...
        endpoints.forEach((ep, i) => {
            ...
            endpoints.forEach((other, j) => {
                if (i === j || connected.has(j)) return;
                if (ep.wallIdx === other.wallIdx) return; // Same wall

                const dist = Math.sqrt((ep.x - other.x) ** 2 + (ep.y - other.y) ** 2);
                if (dist > 0.01 && dist < maxGap) {
                    nearby.push({ idx: j, dist, other });
                }
            });

            // If there are nearby endpoints, connect to the closest one (even if multiple)
            // This handles T-junctions or messy corners better than "exactly one"
            if (nearby.length >= 1) {
                // Sort by distance to find the closest candidate
                nearby.sort((a, b) => a.dist - b.dist);
                const bestMatch = nearby[0];

                // Bridge the gap with a new wall segment
                const other = bestMatch.other;
                newWalls.push({
                    start: { x: ep.x, y: ep.y },
        ...
        // Add new walls to close gaps
        if (newWalls.length > 0) {
            console.log(`[CAD Processor] Closed ${newWalls.length} wall gaps`);
