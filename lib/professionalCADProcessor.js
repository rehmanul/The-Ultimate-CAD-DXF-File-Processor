...
class ProfessionalCADProcessor {
    ...
    _closeWallGaps() {
        ...
        endpoints.forEach((ep, i) => {
            ...
            // If this endpoint has exactly one nearby endpoint, they're probably connected
            // If more than one, it's a junction
            // If zero nearby and at building edge, it might need closing
            if (nearby.length === 1) {
                // Bridge the gap with a new wall segment
                const other = nearby[0].other;
                newWalls.push({
                    start: { x: ep.x, y: ep.y },
