...
class ProfessionalCADProcessor {
    ...
    async processDWG(filePath) {
        ...
        // Load libredwg with explicit wasm path to avoid data: URI issues in Node/docker
        const libredwgPkg = require('@mlightcad/libredwg-web');
        const libredwg = libredwgPkg.default || libredwgPkg;
        const { LibreDwg, createModule, Dwg_File_Type } = libredwg;

        const wasmPath = require.resolve('@mlightcad/libredwg-web/wasm/libredwg-web.wasm');
        const wasm = await createModule({
            locateFile: () => wasmPath
        });
        const dwg = new LibreDwg(wasm);

        const data = dwg.dwg_read_data(buffer, Dwg_File_Type.DWG);
        if (!data) {
            throw new Error('DWG parsing failed (no data returned)');
        }

        const result = dwg.convertEx(data);
        const database = result && result.database ? result.database : null;
        if (!database || !Array.isArray(database.entities)) {
