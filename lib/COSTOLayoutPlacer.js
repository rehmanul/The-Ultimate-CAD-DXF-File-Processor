 ...
 * 
 * Now uses LayerZoneDetector for reliable zone detection:
 * 1. Zones detected from DXF layers (MUR, NO_ENTREE, etc.)
 ...
 */
// LayerZoneDetector is now used in server.js to pre-detect zones
...
class COSTOLayoutPlacer {
    ...
    _getStorageZones() {
        // Priority 1: Use layer-detected zones (from LayerZoneDetector)
        if (this.floorPlan.layerZones && this.floorPlan.layerZones.length > 0) {
            console.log(`[COSTOLayoutPlacer] Using ${this.floorPlan.layerZones.length} layer-detected zones`);
            const filtered = this.floorPlan.layerZones.filter(z => z.area >= this.minZoneArea);
            console.log(`[COSTOLayoutPlacer] After area filter: ${filtered.length} zones`);
            return this._validateZones(filtered);
        }

        // Priority 2: Use pre-detected rooms if available
        ...
        // Priority 3: Fall back to bounds
        console.log('[COSTOLayoutPlacer] No zones detected, will use bounds fallback');
        return [];
    }
