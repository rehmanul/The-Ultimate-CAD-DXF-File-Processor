...
class COSTOLayoutPlacer {
    ...
    _getStorageZones() {
        ...
        if (this.rooms && this.rooms.length > 0) {
            ...
            return this.rooms.filter(r => r.area >= this.minZoneArea);
        }
        ...
        if (this.walls && this.walls.length > 0) {
            ...
            try {
                ...
                // Filter to large zones suitable for storage
                const filtered = detectedRooms.filter(r => r.area >= this.minZoneArea);
                console.log(`[COSTOLayoutPlacer] After filter (minArea=${this.minZoneArea}): ${filtered.length} zones`);
                return filtered;
            } catch (e) {
