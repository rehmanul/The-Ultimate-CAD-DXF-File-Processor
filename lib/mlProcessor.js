...
class MLProcessor {
    ...
    async classifyRoom(roomData) {
        if (!this.isInitialized || !this.models.roomClassifier) {
            throw new Error('Room classifier model is not initialized');
        }
    ...
    /**
     * Extract normalized room-level features for ML models
     */
    extractRoomFeatures(room = {}) {
        ...
        const defaultDim = 5;
        const width = bounds ? Math.max((bounds.maxX || 0) - (bounds.minX || 0), 0) : defaultDim;
        const height = bounds ? Math.max((bounds.maxY || 0) - (bounds.minY || 0), 0) : defaultDim;
        const safeHeight = height === 0 ? 1 : height;
        const aspectRatio = width > 0 && safeHeight > 0 ? width / safeHeight : 1;

        const adjacencyCount = room.adjacency ? Object.keys(room.adjacency).length : 0;

        // Attempt to find entrances from room object or context
        const entrances = room.entrances || (room.floorPlan ? room.floorPlan.entrances : null) || [];

        const distanceToEntrance = room.center
            ? this.calculateDistanceToEntrance(room.center, entrances)
            : 10;
        const perimeter = bounds
            ? 2 * (width + height)
            : 2 * (defaultDim + defaultDim);
    ...
    classifyCADEntity(entityData) {
        if (!this.isInitialized || !this.models.cadEntityClassifier) {
            throw new Error('CAD entity classifier model is not initialized');
        }
        ...
        try {
            ...
            const maxIndex = result.indexOf(Math.max(...result));
            const predictedType = entityTypes[maxIndex];

            const complianceReport = this.checkEntityCompliance(entityData, predictedType);
    ...
    extractCADEntityFeatures(entity) {
        ...
        // Layer name hash (simple hash normalized to always be positive)
        ...
        const positionX = entity.center ? entity.center.x / 10 : 0;
        const positionY = entity.center ? entity.center.y / 10 : 0;
    ...
    calculateDistanceToEntrance(center, entrances) {
        ...
        if (!entrances || entrances.length === 0) {
            return 10; // Default max distance if no entrances found
        }
        ...
        entrances.forEach(entrance => {
            ...
            // Handle different entrance formats
            if (entrance.center) {
                entranceCenter = entrance.center;
            } else if (entrance.start && entrance.end) {
                entranceCenter = {
                    x: (entrance.start.x + entrance.end.x) / 2,
                    y: (entrance.start.y + entrance.end.y) / 2
                };
            } else if (typeof entrance.x === 'number' && typeof entrance.y === 'number') {
                entranceCenter = entrance;
            }
        ...
        return Number.isFinite(minDist) ? minDist : 10;
    }

    /**
     * Disabled in production
     */
    disabledRoomClassification(room) {
        throw new Error('Room classification requires initialized models');
    }

    /**
     * Disabled in production
     */
    disabledCADEntityClassification(entity) {
        throw new Error('CAD entity classification requires initialized models');
    }

    /**
     * Check a CAD entity's compliance with defined standards
     */
    checkEntityCompliance(entity, predictedType) {
        const standardsForType = cadStandards[predictedType];
        if (!standardsForType) {
            return {
                isCompliant: true, // No standards to check against
                issues: [],
            };
        }
        ...
        // Check layer name
        if (!standardsForType.layer.test(entityLayer)) {
            issues.push(`Layer name '${entityLayer}' does not match standard: ${standardsForType.layer}`);
        }

        // Check color
        const standardColor = standardsForType.color;
        const entityR = (entityColor >> 16) & 0xFF;
        const entityG = (entityColor >> 8) & 0xFF;
        const entityB = entityColor & 0xFF;

        if (entityR !== standardColor[0] || entityG !== standardColor[1] || entityB !== standardColor[2]) {
            issues.push(`Color [${entityR},${entityG},${entityB}] does not match standard: [${standardColor.join(',')}]`);
        }
    ...
    async suggestFurniturePlacement(room, furnitureType) {
        if (!this.isInitialized || !this.models.furniturePlacer) {
            throw new Error('Furniture placement model is not initialized');
        }
     ...
     * Disabled in production
     */
    disabledFurniturePlacement(room, furnitureType) {
        throw new Error('Furniture placement requires initialized models');
    }
    ...
    async scoreLayout(layout, floorPlan) {
        if (!this.isInitialized || !this.models.layoutOptimizer) {
            throw new Error('Layout optimizer model is not initialized');
        }
        ...
        try {
            ...
            const rawScore = Array.isArray(result) ? result[0] : 0;
            const boundedScore = Number.isFinite(rawScore) ? Math.min(Math.max(rawScore, 0), 1) : 0;
    ...
    extractLayoutFeatures(layout, floorPlan) {
        ...
        // Basic density
        const totalArea = ilots.reduce((sum, ilot) => sum + (ilot.area || ((ilot.width || 0) * (ilot.height || 0))), 0);
        const boundsArea = floorPlan.bounds
            ? Math.abs((floorPlan.bounds.maxX - floorPlan.bounds.minX) * (floorPlan.bounds.maxY - floorPlan.bounds.minY))
            : 0;
        const floorArea = floorPlan.totalArea || boundsArea || 100;
            ...
            ilots.forEach(ilot => {
                ...
                entrances.forEach(entrance => {
                    const entranceCenter = entrance
                        ...
                        : null;
                    if (entranceCenter) {
                        const dist = Math.hypot(center.x - entranceCenter.x, center.y - entranceCenter.y);
     ...
     * Disabled in production
     */
    disabledLayoutScoring(layout, floorPlan) {
        throw new Error('Layout scoring requires initialized models');
    }
