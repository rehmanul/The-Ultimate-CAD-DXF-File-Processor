...
class ProfessionalGridLayoutEngine {
    constructor(floorPlan, options = {}) {
        ...
        const pairTol = 0.5; // max distance between parallel wall faces
        const wallBuffer = 0.15; // buffer around thick wall bodies
        const internalSegs = this.wallSegs;
        for (let i = 0; i < internalSegs.length; i++) {
            const a = internalSegs[i];
            if (a.len < 1.0) continue; // skip short segments
        ...
        console.log(`[BayGrid] Thick wall pairs detected: ${thickWallCount}`);
        console.log(`[BayGrid] ${this.fzRects.length} FZ, ${this.wallSegs.length} internal walls, ${perimSkipped} perimeter walls skipped, ${this.allWallSegs.length} total wall segs`);
    }
    ...
    generate(config = {}) {
        ...
        const afterWallCheck = units.filter(u => {
            ...
            for (const fz of this.fzRects) {
                // Check if box center is inside any forbidden zone
                if (cx >= fz.x && cx <= fz.x + fz.w &&
                    cy >= fz.y && cy <= fz.y + fz.h) {
                    wallOverlapRemoved++;
                    return false;
                }
            }
        ...
        console.log(`[BayGrid] Post - process: ${clipped} obstacle - clipped, ${wallClipped} wall - clipped, ${removed} invalid - removed, ${enclosedRemoved} enclosed - removed`);
