...
class ProfessionalGridLayoutEngine {
    ...
    generate(config = {}) {
        ...
        console.log(`[BayGrid] Pathway network: ${rowBands.length} aisles, 4 perimeter, ` +
            `${Math.ceil((numCols - 1) / 1)} spines, ${wallConnectors} wall connectors`);
        ...
        const splitCorridorAtWalls = (corr) => {
            const segments = [];
            const obstacles = [...this.fzRects.filter(f => f.type !== "thick_wall" && f.type !== "wall_junction"), ...structuralWalls.map(r => ({
