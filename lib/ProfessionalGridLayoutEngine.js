...
class ProfessionalGridLayoutEngine {
    ...
    generate(config = {}) {
        ...
        const detectEnclosedRooms = () => {
            ...
            for (const h1 of horiz) {
                for (const h2 of horiz) {
                    ...
                    if (hasLeft && hasRight) {
                        const area = (hRight - hLeft) * height;
                        if (area > 1.0 && area < 12) { // only small service rooms (stairwells, elevators, shafts)
        ...
        const maxRayDist = 3.5; // only catch very small enclosed spaces
