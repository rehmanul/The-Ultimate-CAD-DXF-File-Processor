...
const COSTOLayoutPlacer = require('./COSTOLayoutPlacer');
...
const buildingCodePresets = require('./buildingCodePresets');
...
router.post('/api/apply-preset', async (req, res) => {
    try {
        ...
        try {
            normalizedDistribution = normalizeDistribution(preset.distribution);
        } catch (error) {
            return res.status(400).json({
                success: false,
                error: error.message || 'Invalid distribution'
            });
        }
        const generatorOptions = buildGeneratorOptions(workingPlan, preset, options);

        // Support COSTO layout style via options.style = 'COSTO'
        let placer;
        if (options.style === 'COSTO' || preset.style === 'COSTO') {
            console.log('[presetRoutes] Using COSTOLayoutPlacer for organized horizontal rows');
            placer = new COSTOLayoutPlacer(workingPlan, generatorOptions);
        } else {
            placer = new RowBasedIlotPlacer(workingPlan, generatorOptions);
        }
 ...
 */
router.get('/building-code-presets', (req, res) => {
    try {
        const presets = buildingCodePresets.getAll();
        res.json({ success: true, presets });
    } catch (error) {
        console.error('Building code presets error:', error);
