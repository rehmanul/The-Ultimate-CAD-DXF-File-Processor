...
import { FloorPlanRenderer } from './threeRenderer.js?v=1772580000';
    ...
    const initRenderer = () => {
        ...
        const checkSize = () => {
            ...
            try {
                ...
                const rendererReady = !!(renderer
                    && renderer.webglAvailable !== false
                ...
                if (rendererReady) {
                    ...
                    renderer = null;
                    console.warn('Renderer instance created but WebGL path is unavailable.');
                }
            } catch (error) {
            ...
            if (renderer && renderer.webglAvailable === false) {
                showNotification('WebGL unavailable. Babylon renderer disabled.', 'error');
            }
...
function initializeModules() {
    ...
    if (generateIlotsBtn) generateIlotsBtn.onclick = generateAllCosto;
    ...
    if (renderer && typeof renderer.toggle3DMode === 'function' && !renderer.__toggle3DWrapped) {
        const originalToggle = renderer.toggle3DMode.bind(renderer);
        renderer.toggle3DMode = function (enable) {
            const is3D = originalToggle(enable);
            if (editor && typeof editor.updateCamera === 'function') {
                editor.updateCamera(this.is3DMode ? this.perspectiveCamera : this.camera);
            }
            return is3D;
        };
        renderer.__toggle3DWrapped = true;
    }
    ...
    // Seed default distribution values once so generation starts from a valid state.
    const defaultDistributionSeed = {
        '0-1': 10,
        '1-3': 25,
        '3-5': 30,
        '5-10': 35
    ...
    const ranges = ['0-1', '1-3', '3-5', '5-10'];
    const shouldSeedDefaults = Array.from(distributionInputs).every(input => !(String(input.value || '').trim()));
    if (shouldSeedDefaults) {
        distributionInputs.forEach((input, index) => {
            const key = ranges[index];
            input.value = String(defaultDistributionSeed[key] ?? 0);
        });
    }
    updateDistributionTotal();
    ...
    distributionInputs.forEach((input, index) => {
        seededDistribution[ranges[index]] = parseFloat(input.value) || 0;
    });
    try {
        distributionEditor.value = JSON.stringify(normalizePresetDistribution(seededDistribution), null, 2);
    } catch (error) {
        distributionEditor.value = JSON.stringify(normalizePresetDistribution(defaultDistributionSeed), null, 2);
    }
    ...
    window.exportHighRes = async function () {
        if (!renderer) return;
    ...
    // Remove legacy controls that caused orientation confusion and raw-plan misuse.
    const removeById = (id) => {
    ...
    if (nav) {
        nav.querySelectorAll('button').forEach((btn) => {
            ...
            if (text.includes('complete raw plan') || text.includes('analyze plan')) {
                btn.remove();
            }
        });
    }

    // Kept for compatibility with older flow callers.
        ...
        container.addEventListener('click', (e) => {
            ...
            walls.forEach(wall => {
                if (wall.start && wall.end) {
                    ...
                    const ndx = dx / wallLength, ndy = dy / wallLength;

                    // Project mouse point onto wall line
                    const tmx = mousePoint.x - sx, tmy = mousePoint.y - sy;
                    const projection = tmx * ndx + tmy * ndy;
                    const clampedProjection = Math.max(0, Math.min(projection, wallLength));
                    const cpx = sx + ndx * clampedProjection, cpy = sy + ndy * clampedProjection;

                    const distance = Math.hypot(mousePoint.x - cpx, mousePoint.y - cpy);
                    if (distance < nearestDistance && distance < 2.0) { // Within 2m of wall
                        nearestDistance = distance;
                        nearestWall = wall;
                        nearestPoint = { x: cpx, y: cpy };
                    }
                }
            });
    ...
    if (exportGltfBtn) {
        exportGltfBtn.addEventListener('click', () => {
            if (!renderer || typeof renderer.downloadGLTF !== 'function') {
                showNotification('GLTF export is not available in this renderer build.', 'warning');
                return;
            }
    ...
    const computeExportCaptureSize = (targetRenderer, longSide = 4600) => {
        ...
        const width = Math.max(1200, Math.round(viewWidth * scale));
        const height = Math.max(700, Math.round(viewHeight * scale));
        return { width, height };
    };

    const captureRendererForExport = async (targetRenderer, longSide = 4600) => {
        const { width, height } = computeExportCaptureSize(targetRenderer, longSide);
        if (typeof targetRenderer.captureImageData === 'function') {
            return targetRenderer.captureImageData(width, height, true);
        }
        return targetRenderer.exportImage(width, height, {
            download: false,
            useActiveCamera: true
        });
    };
    ...
    if (exportDxfBtn) {
        exportDxfBtn.addEventListener('click', () => {
            if (!professionalExport || typeof professionalExport.downloadDXF !== 'function') {
                showNotification('DXF export is not available right now.', 'warning');
                return;
            }
            if (!currentFloorPlan) {
                showNotification('No floor plan loaded', 'warning');
    ...
    if (export4kBtn) {
        export4kBtn.addEventListener('click', async () => {
            if (!professionalExport || typeof professionalExport.downloadHighResPNG !== 'function') {
                showNotification('4K export is not available right now.', 'warning');
                return;
            }
            try {
                ...
                showNotification('4K export failed: ' + error.message, 'error');
            }
        });
    }
        ...
        optimizePathsBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (j && Array.isArray(j.corridors)) {
                    ...
                    window._costoCirculationPaths = Array.isArray(j.circulationPaths)
                        ? j.circulationPaths
                        : (Array.isArray(j.flowPaths) ? j.flowPaths : []);
...
async function handleFileUpload(e) {
    ...
    try {
        ...
        currentFloorPlan = {
            ...
            envelope: analysisData.envelope || [],
            dimensions: analysisData.dimensions || [],
            annotations: analysisData.annotations || [],
            functionalAreas: analysisData.functionalAreas || [],
            specialRooms: analysisData.specialRooms || [],
            greenZones: analysisData.greenZones || [],
            ...
            _rawAnalysis: analysisData._rawAnalysis || null,
            _syntheticSegments: analysisData._syntheticSegments || [],
        ...
        if (renderer) {
            try {
                ...
                // Ensure all primary layers are visible after each new upload.
                const layerIds = ['layerWalls', 'layerEntrances', 'layerForbidden', 'layerIlots', 'layerCorridors', 'layerArrows'];
                const layerNames = ['walls', 'entrances', 'forbidden', 'ilots', 'corridors', 'arrows'];
                layerIds.forEach((id, idx) => {
                    const cb = document.getElementById(id);
                    if (cb) cb.checked = true;
                    if (typeof renderer.toggleLayer === 'function') {
                        renderer.toggleLayer(layerNames[idx], true);
                    }
                });
                ...
                setTimeout(() => {
                    try {
                        if (!fitCurrentView() && currentFloorPlan.bounds) {
...
function renderCurrentState() {
    ...
    try {
        ...
        // If COSTO layout data is available, use unified renderer path.
        const costoRadiators = Array.isArray(window._costoRadiators) ? window._costoRadiators : [];
        const costoCirculationPaths = Array.isArray(window._costoCirculationPaths) ? window._costoCirculationPaths : [];
        const hasCostoData = generatedIlots.length > 0 && (
            (Array.isArray(corridorNetwork) && corridorNetwork.length > 0) ||
            costoRadiators.length > 0 ||
            costoCirculationPaths.length > 0
        );
        if (hasCostoData && currentFloorPlan && renderer.renderCostoLayout) {
            ...
            if (multiFloorResult && stackVisualizationEnabled) {
                updateConnectorVisualization();
            } else if (renderer.renderConnectors) {
        ...
        // Non-COSTO path: load raw floor plan + overlay
        if (currentFloorPlan) {
            renderer.loadFloorPlan(currentFloorPlan);
        }
        ...
        // Always draw fire-map overlay if possible, even for legacy path
        // DISABLED: navigation overlay creates duplicate flow rendering
        //     renderer._drawNavigationOverlay(currentFloorPlan || {}, {
        //     units: generatedIlots || [],
        //     corridors: corridorNetwork || [],
...
function toCostoDistribution(normalizedDistribution) {
    ...
    if (!normalizedDistribution || typeof normalizedDistribution !== 'object') {
        return { ...bucketWeights };
    }

    Object.entries(normalizedDistribution).forEach(([range, rawWeight]) => {
        const weight = Number(rawWeight);
        if (!Number.isFinite(weight) || weight <= 0) return;
        const [rawMin, rawMax] = String(range).split('-');
        const min = Number(rawMin);
        const max = Number(rawMax);
        const center = Number.isFinite(min) && Number.isFinite(max)
            ? (min + max) / 2
            : (Number.isFinite(min) ? min : 0);

        if (center <= 2.0) bucketWeights.small += weight;
        else if (center <= 5.0) bucketWeights.medium += weight;
        else if (center <= 10.0) bucketWeights.large += weight;
        else bucketWeights.xlarge += weight;
    });

    const toPercent = (value) => Math.max(0, Math.round(value * 1000) / 10);
    return {
        small: toPercent(bucketWeights.small),
        medium: toPercent(bucketWeights.medium),
        large: toPercent(bucketWeights.large),
        xlarge: toPercent(bucketWeights.xlarge)
    };
}
...
async function generateAllCosto() {
    ...
    try {
        ...
        const floorWidth = Number(bounds.maxX) - Number(bounds.minX);
        const floorHeight = Number(bounds.maxY) - Number(bounds.minY);
        const floorArea = Math.max(0, floorWidth * floorHeight);
        const targetInput = document.getElementById('targetIlotsInput');
        const userTarget = targetInput && targetInput.value.trim() ? parseInt(targetInput.value, 10) : NaN;
        const autoTargetIlots = Math.max(80, Math.min(500, Math.floor((floorArea * 0.86) / 3.4)));
        const targetIlots = Number.isFinite(userTarget) && userTarget > 0
            ? Math.max(20, Math.min(500, userTarget))
            : autoTargetIlots;
        ...
        showLoader('Generating layout...', 30);
        ...
        // Render everything through a unified COSTO path.
        ...
        if (typeof updateCorridorStatistics === 'function') {
            updateCorridorStatistics(corridorNetwork);
        }
        updateStats();
...
async function generateIlots() {
    ...
    try {
        ...
        const autoTargetIlots = Math.max(60, Math.min(500, Math.floor((floorArea * 0.84) / 3.6)));
        ...
        if (data.useCostoCorridors && data.costoCorridors && data.costoCorridors.length > 0) {
            ...
            // Non-COSTO path: use piecemeal rendering
            setTimeout(() => {
                renderCurrentState();
                updateLegendTable();
            }, 500);
...
function parseDistribution() {
    ...
    if (distributionInputs.length > 0) {
        const distribution = {};
        const ranges = ['0-2', '2-5', '5-10', '10-20'];
        let hasValues = false;
    ...
    try {
        ...
        if (corridorNetwork && corridorNetwork.length > 0) {
            ...
            if (renderer) {
                ...
                // DISABLED: radiator zigzags create visual clutter
                // renderer.renderRadiators(radiators);
                // }

                const circPaths = window._costoCirculationPaths || [];
                // DISABLED: old circulation dashes replaced by global flow arrows
...
function initializeLayoutChrome() {
    const layoutRoot = document.querySelector('.app-layout') || document.querySelector('.app-container');
...
function showLoader(message, percentage = 0) {
    try {
        ...
        const loaderMsg = document.getElementById('loaderMessage') || document.getElementById('loadingMessage');
...
function initializeAutoHide() {
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        if (!leftPinned) {
            const leftWidth = leftSidebar ? Math.max(180, leftSidebar.offsetWidth || 260) : 260;
            if (clientX < 30) {
                const wasHidden = leftSidebar.classList.contains('auto-hidden');
                leftSidebar.classList.remove('auto-hidden');
                if (wasHidden) resizeCanvas();
            } else if (clientX > leftWidth + 36) {
                clearTimeout(hideTimer);
                hideTimer = setTimeout(() => {
                    if (!leftPinned) {
        ...
        if (!rightPinned) {
            const rightWidth = rightSidebar ? Math.max(180, rightSidebar.offsetWidth || 260) : 260;
            if (clientX > windowWidth - 30) {
                const wasHidden = rightSidebar.classList.contains('auto-hidden');
                rightSidebar.classList.remove('auto-hidden');
                if (wasHidden) resizeCanvas();
            } else if (clientX < windowWidth - rightWidth - 36) {
                clearTimeout(hideTimer);
                hideTimer = setTimeout(() => {
                    if (!rightPinned) {
...
function initializeResizableDrawers() {
    const leftSidebar = document.querySelector('.sidebar-left');
    const rightSidebar = document.querySelector('.sidebar-right');
    ...
    const minWidth = 170;
    const maxWidth = 420;
    const widthPresets = [200, 240, 280, 320, 360];
    const mobileBreakpoint = 768;
    const clamp = (value, min, max) => Math.max(min, Math.min(max, value));

    const notifyResize = () => {
        if (!renderer || typeof renderer.onResize !== 'function') return;
        window.requestAnimationFrame(() => renderer.onResize());
    };

    const readStoredWidth = (key, fallbackPx) => {
        if (window.innerWidth <= mobileBreakpoint) return fallbackPx;
        const raw = Number(window.localStorage.getItem(key));
        if (!Number.isFinite(raw)) return fallbackPx;
        return clamp(raw, minWidth, maxWidth);
    };

    const setWidth = (side, widthPx) => {
        const safe = clamp(Math.round(widthPx), minWidth, maxWidth);
        const cssVar = side === 'left' ? '--sidebar-left-width' : '--sidebar-right-width';
        const storageKey = side === 'left' ? 'fp.drawer.left.width' : 'fp.drawer.right.width';
        root.style.setProperty(cssVar, `${safe}px`);
        window.localStorage.setItem(storageKey, String(safe));
        notifyResize();
    };

    const getCurrentWidth = (side, sidebarEl) => {
        const cssVar = side === 'left' ? '--sidebar-left-width' : '--sidebar-right-width';
        const fromCss = parseFloat(getComputedStyle(root).getPropertyValue(cssVar));
        if (Number.isFinite(fromCss) && fromCss > 0) return fromCss;
        return sidebarEl ? sidebarEl.getBoundingClientRect().width : 240;
    };

    if (leftSidebar) {
        const fallback = leftSidebar.getBoundingClientRect().width || 240;
        setWidth('left', readStoredWidth('fp.drawer.left.width', fallback));
    }
    if (rightSidebar) {
        const fallback = rightSidebar.getBoundingClientRect().width || 240;
        setWidth('right', readStoredWidth('fp.drawer.right.width', fallback));
    }

    const setupHandle = (sidebarEl, side) => {
        if (!sidebarEl) return;
        if (sidebarEl.querySelector('.sidebar-resize-handle')) return;

        const handle = document.createElement('button');
        handle.type = 'button';
        handle.className = 'sidebar-resize-handle';
        handle.setAttribute('aria-label', `Resize ${side} drawer`);
        handle.title = `Resize ${side} drawer`;
        ...
        handle.addEventListener('pointerdown', (event) => {
            ...
            startWidth = getCurrentWidth(side, sidebarEl);
            moved = false;
        ...
        handle.addEventListener('click', (event) => {
            ...
            const current = getCurrentWidth(side, sidebarEl);
            let next = widthPresets[0];
            const idx = widthPresets.findIndex((value) => value >= current - 2 && value <= current + 2);
            if (idx >= 0) next = widthPresets[(idx + 1) % widthPresets.length];
            else {
                const nearest = widthPresets.reduce((acc, value) => Math.abs(value - current) < Math.abs(acc - current) ? value : acc, widthPresets[0]);
                const nearestIdx = Math.max(0, widthPresets.indexOf(nearest));
                next = widthPresets[(nearestIdx + 1) % widthPresets.length];
            }
            setWidth(side, next);
        });
    };

    setupHandle(leftSidebar, 'left');
    setupHandle(rightSidebar, 'right');

    window.addEventListener('resize', () => {
        if (window.innerWidth <= mobileBreakpoint) {
            root.style.setProperty('--sidebar-left-width', '280px');
            root.style.setProperty('--sidebar-right-width', '280px');
            notifyResize();
            return;
        }
        if (leftSidebar) setWidth('left', getCurrentWidth('left', leftSidebar));
        if (rightSidebar) setWidth('right', getCurrentWidth('right', rightSidebar));
    });
}
...
// Raw-plan and manual corridor controls are removed from the production UI.
if (topGenerateCorridorsBtn) topGenerateCorridorsBtn.remove();
if (analyzeRawPlanBtn) analyzeRawPlanBtn.remove();
if (completeRawPlanBtn) completeRawPlanBtn.remove();
...
if (topGenerateIlotsBtn) {
    topGenerateIlotsBtn.addEventListener('click', () => {
        ...
        if (sidebarBtn) {
            ...
            if (typeof window.generateAllCosto === 'function') {
                window.generateAllCosto();
            }
        }
    });
}
...
function getRenderableBounds() {
    const planBounds = currentFloorPlan && currentFloorPlan.bounds ? currentFloorPlan.bounds : null;
    const hasPlanBounds = planBounds &&
        Number.isFinite(planBounds.minX) &&
        Number.isFinite(planBounds.minY) &&
        Number.isFinite(planBounds.maxX) &&
        Number.isFinite(planBounds.maxY) &&
    ...
    if (Array.isArray(currentFloorPlan?.walls)) {
        currentFloorPlan.walls.forEach((w) => {
            if (w?.start && w?.end) {
                pushPoint(w.start.x, w.start.y);
                pushPoint(w.end.x, w.end.y);
            } else {
                pushPath(w?.polygon);
            }
        });
    }
    if (Array.isArray(currentFloorPlan?.envelope)) {
        currentFloorPlan.envelope.forEach((w) => {
            if (w?.start && w?.end) {
                pushPoint(w.start.x, w.start.y);
                pushPoint(w.end.x, w.end.y);
            }
    ...
    if (Array.isArray(generatedIlots)) {
        generatedIlots.forEach((ilot) => pushRect(Number(ilot.x), Number(ilot.y), Number(ilot.width), Number(ilot.height)));
    }
    if (Array.isArray(corridorNetwork)) {
        corridorNetwork.forEach((c) => {
            pushRect(Number(c.x), Number(c.y), Number(c.width), Number(c.height));
            pushPath(c.path || c.corners || c.polygon);
        });
    }
    if (Array.isArray(window._costoCirculationPaths)) {
        window._costoCirculationPaths.forEach((cp) => pushPath(cp.path));
    }
    if (Array.isArray(window._costoRadiators)) {
        window._costoRadiators.forEach((r) => pushPath(r.path));
    }

    if (points.length === 0) {
        return hasPlanBounds ? planBounds : null;
    }
    ...
    if (hasPlanBounds) {
        minX = Math.max(minX, planBounds.minX);
        minY = Math.max(minY, planBounds.minY);
        maxX = Math.min(maxX, planBounds.maxX);
        maxY = Math.min(maxY, planBounds.maxY);
    }

    return (maxX > minX && maxY > minY)
        ? { minX, minY, maxX, maxY }
        : (hasPlanBounds ? planBounds : null);
}

function fitCurrentView() {
    if (!renderer || typeof renderer.fitToBounds !== 'function') return false;
...
if (resetCameraBtn) {
    resetCameraBtn.addEventListener('click', () => {
        if (!renderer) return;
        if (typeof renderer.resetView === 'function') {
            renderer.resetView();
            if (typeof renderer.render === 'function') renderer.render();
            showNotification('Camera reset to default', 'info');
            return;
        }
        if (renderer.controls && typeof renderer.controls.reset === 'function') {
...
if (gridToggleBtnEl) {
    gridToggleBtnEl.addEventListener('click', () => {
        ...
        if (renderer) {
            if (gridVisible && typeof renderer.showGrid === 'function') {
                renderer.showGrid();
            } else if (!gridVisible && typeof renderer.hideGrid === 'function') {
                renderer.hideGrid();
            } else if (renderer.gridHelper) {
...
// Browser smoke-test/debug bridge (read-only diagnostics + safe actions).
// Exposed only on local hosts unless explicitly enabled.
if (typeof window !== 'undefined') {
    window.generateAllCosto = generateAllCosto;
    window.generateIlots = generateAllCosto;
}

if (typeof window !== 'undefined' && (
    ['localhost', '127.0.0.1'].includes(window.location.hostname) ||
    window.__ENABLE_FLOORPLAN_DEBUG__ === true
)) {
    window.__floorplanDebug = {
        hasFloorPlan: () => !!currentFloorPlan,
        wallCount: () => (Array.isArray(currentFloorPlan?.walls) ? currentFloorPlan.walls.length : 0),
        wallShapeStats: () => {
            const walls = Array.isArray(currentFloorPlan?.walls) ? currentFloorPlan.walls : [];
            let withStartEnd = 0;
            let withPolygon = 0;
            let polyGt2 = 0;
            for (const w of walls) {
                if (w?.start && w?.end) withStartEnd += 1;
                if (Array.isArray(w?.polygon) && w.polygon.length >= 2) {
                    withPolygon += 1;
                    if (w.polygon.length > 2) polyGt2 += 1;
                }
            }
            return { total: walls.length, withStartEnd, withPolygon, polyGt2 };
        },
        wallSamples: (limit = 5) => {
            const walls = Array.isArray(currentFloorPlan?.walls) ? currentFloorPlan.walls : [];
            return walls.slice(0, Math.max(1, Math.min(50, Number(limit) || 5)));
        },
        ...
        isGridVisible: () => {
            ...
            if (renderer.defaultGrid && typeof renderer.defaultGrid.visible === 'boolean') {
                anyVisible = anyVisible || !!renderer.defaultGrid.visible;
            }

            // Three.js renderer path
            if (renderer.gridHelper && typeof renderer.gridHelper.visible === 'boolean') {
                anyVisible = anyVisible || !!renderer.gridHelper.visible;
            }

            // Legacy/Babylon compatibility path
            if (!renderer._gridLines) return anyVisible;
            if (typeof renderer._gridLines.isEnabled === 'function') {
                return anyVisible || !!renderer._gridLines.isEnabled();
            }
            return anyVisible || !!renderer._gridLines.isVisible;
        },
        getLayerState: () => ({
            walls: !!renderer?.wallsGroup?.isEnabled?.(),
            entrances: !!renderer?.entrancesGroup?.isEnabled?.(),
            forbidden: !!renderer?.forbiddenGroup?.isEnabled?.(),
            ilots: !!renderer?.ilotsGroup?.isEnabled?.(),
            corridors: !!renderer?.corridorsGroup?.isEnabled?.(),
            arrows: !!renderer?.corridorArrowsGroup?.isEnabled?.()
        }),
        getCameraState: () => {
            if (!renderer || !renderer.camera) return null;
            return {
                position: renderer.camera.position ? { x: renderer.camera.position.x, y: renderer.camera.position.y, z: renderer.camera.position.z } : null,
                target: (typeof renderer.camera.getTarget === 'function')
                    ? (() => {
                        const t = renderer.camera.getTarget();
                        return { x: t.x, y: t.y, z: t.z };
                    })()
                    : null,
                orthoLeft: renderer.camera.orthoLeft,
                orthoRight: renderer.camera.orthoRight,
                orthoTop: renderer.camera.orthoTop,
                orthoBottom: renderer.camera.orthoBottom,
                minZ: renderer.camera.minZ,
                maxZ: renderer.camera.maxZ
            };
        },
        getRenderableBounds: () => getRenderableBounds(),
        getPlanBounds: () => (currentFloorPlan && currentFloorPlan.bounds ? currentFloorPlan.bounds : null),
        getRendererInfo: () => (renderer && typeof renderer.getRendererInfo === 'function' ? renderer.getRendererInfo() : null),
        getWallMeshCount: () => {
            if (!renderer || !renderer.wallsGroup) return 0;
            if (typeof renderer.wallsGroup.getChildMeshes === 'function') {
                return renderer.wallsGroup.getChildMeshes(false).length;
            }
            if (Array.isArray(renderer.wallsGroup.children)) return renderer.wallsGroup.children.length;
            return 0;
        },
