...
class CostoExports {
    ...
    _collectGeometryPoints(floorPlan, solution) {
        ...
        corridors.forEach((corridor) => {
            ...
            pushPath(corridor && (corridor.path || corridor.corners || corridor.polygon));
        });

        const circulationPaths = solution && Array.isArray(solution.circulationPaths) ? solution.circulationPaths : [];
        circulationPaths.forEach((cp) => pushPath(cp && cp.path));

        const radiators = solution && Array.isArray(solution.radiators) ? solution.radiators : [];
        radiators.forEach((rad) => {
            if (Array.isArray(rad && rad.positions)) {
                rad.positions.forEach((p) => pushPoint(Number(p && p.x), Number(p && p.y)));
            }
            pushPath(rad && rad.path);
        });
    ...
    _computeEffectiveBounds(floorPlan, solution, options = {}) {
        ...
        if (fallbackWidth > 0 && fallbackHeight > 0) {
            ...
            if (widthCoverage < minCoverage || heightCoverage < minCoverage) {
                ...
                if (!Number.isFinite(fallbackDiag) || !Number.isFinite(computedDiag) || fallbackDiag <= computedDiag * 2.2) {
                    return fallback;
                }
            }
        }
    ...
    _resolveReferencePageSize(pageSize, orientation, planWidth, planHeight, margin, legendWidth, titleBlockHeight, multiFloor = false) {
        ...
        const score = (dims) => {
            ...
            const floorHeight = multiFloor ? (drawHeight / 2) : drawHeight;
            const scale = Math.min(drawWidth / Math.max(planWidth, 1e-6), floorHeight / Math.max(planHeight, 1e-6));
            const scaledW = planWidth * scale;
            const scaledH = planHeight * scale;
            return (scaledW * scaledH) / (drawWidth * floorHeight);
        };
...
`;
    ...
    async drawReferenceLegend(page, x, y, width, height, options = {}) {
        const labelFont = await page.doc.embedFont(StandardFonts.HelveticaOblique);
        const panelPad = 14;
        ...
        if (options.includeCompass) {
            await this.drawCompassRose(page, x + width - 38, y + height - 58, 34);
        }
    }
...
`;
    ...
    async exportToReferencePDF(solution, floorPlan, metrics, options = {}) {
        ...
        const fitScaleFactor = Number.isFinite(Number(fitFactor))
            ? Math.max(0.8, Math.min(1, Number(fitFactor)))
            : 0.97;

        const renderFloorPlan = {
            ...floorPlan,
            bounds: this._computeEffectiveBounds(floorPlan, solution, options)
        };
        ...
        if (canMultiFloor) {
            for (let i = 0; i < 2; i++) {
                ...
                renderFloorPlans.push({
                    ...fp,
                    bounds: this._computeEffectiveBounds(fp, sol, options)
                });
        ...
        if (canMultiFloor) {
            ...
            const scaleFactor = Math.min(availableWidth / maxPlanWidth, floorHeight / maxPlanHeight) * fitScaleFactor;

            const width1 = (b1.maxX - b1.minX) * scaleFactor;
            const height1 = (b1.maxY - b1.minY) * scaleFactor;
            const width2 = (b2.maxX - b2.minX) * scaleFactor;
            const height2 = (b2.maxY - b2.minY) * scaleFactor;

            const topOffsetX = offsetX + Math.max(0, (availableWidth - width1) / 2);
            const topOffsetY = offsetY + floorHeight + Math.max(0, (floorHeight - height1) / 2);
            const bottomOffsetX = offsetX + Math.max(0, (availableWidth - width2) / 2);
            const bottomOffsetY = offsetY + Math.max(0, (floorHeight - height2) / 2);
    ...
    exportToReferenceSVG(solution, floorPlan, metrics, options = {}) {
        ...
        if (canMultiFloor) {
            ...
            const maxPlanHeight = Math.max(b1.maxY - b1.minY, b2.maxY - b2.minY);
            const floorHeight = availableHeight / 2;
            scaleFactor = Math.min(availableWidth / maxPlanWidth, floorHeight / maxPlanHeight) * fitScaleFactor;
        } else {
        ...
        if (canMultiFloor) {
            ...
            renderFloor(renderFloorPlans[0], renderSolutions[0], topOffsetX, topOffsetY, 'PLAN ETAGE 01');
            renderFloor(renderFloorPlans[1], renderSolutions[1], bottomOffsetX, bottomOffsetY, 'PLAN ETAGE 02');
        } else {
    ...
    async drawEnhancedFloorPlanToPDF(page, floorPlan, solution, scale, offsetX, offsetY, options = {}) {
        ...
        const WALL_THICKNESS = 1.5;  // Thin line like reference
        ...
        const ENVELOPE_THICKNESS = 3; // Medium thickness for envelope
        ...
        boxes.forEach(box => {
            ...
            const getEdgeWidth = (type) => type === 'tole_blanche' ? 0.5 : 1.8;
