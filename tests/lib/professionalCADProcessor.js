...
const { parseInsunits, getScaleToMeters, inferScaleFromGeometry, normalizeToMeters } = require('./units');
...
function resolveLibreDwgWasmDir() {
    ...
    // Use node_modules path when package entry resolution is unavailable.
    const nodeModulesWasmDir = path.normalize(path.join(process.cwd(), 'node_modules', '@mlightcad', 'libredwg-web', 'wasm'));
    cachedWasmDir = nodeModulesWasmDir;
    return cachedWasmDir;
}
...
class ProfessionalCADProcessor {
    ...
    async processDXF(filePath) {
        ...
        // Detect binary DXF (AutoCAD Binary DXF) - dxf-parser only supports ASCII DXF
        const header = buffer.slice(0, 22).toString('ascii');
        if (header.startsWith('AutoCAD Binary DXF')) {
            const baseName = path.basename(filePath, '.dxf');
            const dwgPath = path.join(path.dirname(filePath), baseName + '.dwg');
            throw new Error(
                `Binary DXF file detected. The DXF parser only supports ASCII DXF. ` +
                `Please either: (1) Use the DWG file instead (e.g. ${path.basename(dwgPath)}) if available, or ` +
                `(2) Re-export from CAD as ASCII DXF (Save As → DXF → ASCII / R12/LT2 format).`
            );
        }
        ...
        return await this._processParsedDXF(dxf, { rawContent: content });
    }
    ...
    async _processParsedDXF(dxf, options = {}) {
        let result = await dxfProcessor.processParsedDXF(dxf);
        // Store raw entities for layer-based zone detection and optional detailed rendering
        let rawEntities = dxf.entities || [];

        let scaleFactor = 1;
        const inferredScale = inferScaleFromGeometry(result.bounds, result.walls);
        const width = Math.abs((result.bounds.maxX || 0) - (result.bounds.minX || 0));
        const height = Math.abs((result.bounds.maxY || 0) - (result.bounds.minY || 0));
        const maxDim = Math.max(width, height);

        if (options && typeof options.rawContent === 'string') {
            const insunits = parseInsunits(options.rawContent);
            const unitScale = getScaleToMeters(insunits);
            const unitScaled = Number.isFinite(unitScale) ? maxDim * unitScale : NaN;
            const inferredScaled = Number.isFinite(inferredScale) ? maxDim * inferredScale : NaN;
            const unitLooksReasonable = Number.isFinite(unitScaled) && unitScaled >= 2 && unitScaled <= 1000;
            const inferredLooksReasonable = Number.isFinite(inferredScaled) && inferredScaled >= 2 && inferredScaled <= 1000;

            if (insunits === 0 || !Number.isFinite(unitScale)) {
                scaleFactor = inferredScale;
            } else if (!unitLooksReasonable && inferredLooksReasonable) {
                console.warn('[Units] INSUNITS scale looks inconsistent with geometry; using inferred scale.');
                scaleFactor = inferredScale;
            } else {
        ...
        if (scaleFactor !== 1) {
            result = normalizeToMeters(result, scaleFactor);

            const scalePoint = (pt) => {
                if (!pt) return pt;
                if (Array.isArray(pt)) {
                    if (pt.length >= 2) return [Number(pt[0]) * scaleFactor, Number(pt[1]) * scaleFactor];
                    return pt;
                }
                if (typeof pt.x === 'number' && typeof pt.y === 'number') {
                    return { ...pt, x: pt.x * scaleFactor, y: pt.y * scaleFactor };
                }
                return pt;
            };
            ...
            const scalePath = (path) => {
                ...
                return path.map((pt) => scalePoint(pt));
            };

            rawEntities = rawEntities.map((ent) => {
                if (!ent || typeof ent !== 'object') return ent;
                const scaled = { ...ent };
                if (Array.isArray(ent.vertices)) scaled.vertices = scalePath(ent.vertices);
                if (Array.isArray(ent.points)) scaled.points = scalePath(ent.points);
                if (ent.start) scaled.start = scalePoint(ent.start);
                if (ent.end) scaled.end = scalePoint(ent.end);
                if (ent.startPoint) scaled.startPoint = scalePoint(ent.startPoint);
                if (ent.endPoint) scaled.endPoint = scalePoint(ent.endPoint);
                if (ent.position) scaled.position = scalePoint(ent.position);
                if (ent.insertionPoint) scaled.insertionPoint = scalePoint(ent.insertionPoint);
                if (ent.center) scaled.center = scalePoint(ent.center);
                if (typeof ent.radius === 'number') scaled.radius = ent.radius * scaleFactor;
                return scaled;
            });
            ...
            if (Array.isArray(result.functionalAreas)) {
                result.functionalAreas = result.functionalAreas.map((a) => ({
                    ...a,
                    position: scalePoint(a.position)
                }));
            }
            if (Array.isArray(result.specialRooms)) {
                result.specialRooms = result.specialRooms.map((room) => ({
                    ...room,
                    position: scalePoint(room.position),
                    polygon: Array.isArray(room.polygon) ? scalePath(room.polygon) : room.polygon
                }));
            }
            if (Array.isArray(result.greenZones)) {
                result.greenZones = result.greenZones.map((zone) => ({
                    ...zone,
                    position: scalePoint(zone.position),
                    polygon: Array.isArray(zone.polygon) ? scalePath(zone.polygon) : zone.polygon
                }));
            }
            if (Array.isArray(result.envelope)) {
                result.envelope = result.envelope.map((seg) => ({
                    ...seg,
        ...
        this.functionalAreas = result.functionalAreas || [];
        this.specialRooms = result.specialRooms || [];
        this.greenZones = result.greenZones || [];

        // Do not auto-complete wall gaps in the base CAD processor.
        // Raw geometry must remain source-authentic unless explicitly requested.
        ...
        return {
            ...
            bounds: this.bounds,
            entities: rawEntities, // Raw DXF entities for LayerZoneDetector
            layers: [],
            metadata: {
    ...
    /**
     * Close wall gaps by finding endpoints that are close but not connected
     * and adding new wall segments to bridge them
     */
    _closeWallGaps() {
        if (!Array.isArray(this.walls) || this.walls.length === 0) return;

        const width = Math.abs((this.bounds.maxX || 0) - (this.bounds.minX || 0));
        const height = Math.abs((this.bounds.maxY || 0) - (this.bounds.minY || 0));
        const diagonal = Math.hypot(width, height);
        const maxGap = Math.max(0.02, Math.min(0.2, diagonal * 0.002));
        if (!Number.isFinite(maxGap) || maxGap <= 0) return;

        const endpoints = [];
        this.walls.forEach((wall, idx) => {
            if (wall.start && wall.end) {
                endpoints.push({ x: wall.start.x, y: wall.start.y, wallIdx: idx, isStart: true });
                endpoints.push({ x: wall.end.x, y: wall.end.y, wallIdx: idx, isStart: false });
            }
        });

        const cellSize = maxGap;
        const buckets = new Map();
        const bucketKey = (x, y) => `${Math.floor(x / cellSize)}_${Math.floor(y / cellSize)}`;
        endpoints.forEach((ep, idx) => {
            const key = bucketKey(ep.x, ep.y);
        ...
        const angleCos = (a, b) => {
            ...
            return Math.abs((a.x * b.x + a.y * b.y) / (lenA * lenB));
        };

        endpoints.forEach((ep, idx) => {
            if (used.has(idx)) return;
            const wall = this.walls[ep.wallIdx];
            ...
            const cx = Math.floor(ep.x / cellSize);
            const cy = Math.floor(ep.y / cellSize);
            let best = null;
            ...
            for (let dx = -1; dx <= 1; dx++) {
                for (let dy = -1; dy <= 1; dy++) {
                    ...
                    for (const j of bucket) {
                        ...
                        if (ep.wallIdx === other.wallIdx) continue;
                        const dist = Math.hypot(ep.x - other.x, ep.y - other.y);
                        if (dist <= 0.001 || dist > maxGap) continue;
                        if (!best || dist < best.dist) {
                            best = { idx: j, dist, other };
                        }
                    }
                }
            }
            ...
            if (!best) return;
            const otherWall = this.walls[best.other.wallIdx];
            if (!otherWall || !otherWall.start || !otherWall.end) return;

            const otherDir = {
                x: otherWall.end.x - otherWall.start.x,
                y: otherWall.end.y - otherWall.start.y
            };
            const cos = angleCos(segDir, otherDir);
            const gapVec = { x: best.other.x - ep.x, y: best.other.y - ep.y };
            const gapCos = angleCos(segDir, gapVec);

            const isParallel = cos > 0.95;
            const isPerpendicular = cos < 0.2;
            if (isParallel && gapCos < 0.8) return;
            if (!isParallel && !isPerpendicular) return;

            const midX = (ep.x + best.other.x) / 2;
            const midY = (ep.y + best.other.y) / 2;

            const applySnap = (endpoint) => {
                const targetWall = this.walls[endpoint.wallIdx];
                if (!targetWall) return;
                if (endpoint.isStart) {
                    targetWall.start = { x: midX, y: midY };
                } else {
                    targetWall.end = { x: midX, y: midY };
                }
            };

            applySnap(ep);
            applySnap(best.other);
            used.add(idx);
            used.add(best.idx);
        });
    }

    _manualParseDisabled(content) {
        throw new Error('Manual DXF parsing is disabled in production');
    ...
    extractRooms(polygons) {
        ...
        if (this.rooms.length === 0) {
            ...
            try {
                ...
                const detectedRooms = roomDetector.detectRooms(this.walls, this.entrances, this.forbiddenZones, this.bounds);
