...
class RoomDetector {
    ...
    async initializeML() {
        ...
        console.warn('[RoomDetector] Geometric engine active (ML augmentation disabled by configuration).');
        this.mlProcessor = null;
    }
    ...
    detectRooms(walls, entrances, forbiddenZones, bounds, options = {}) {
        ...
        // Sanitize polygons: filter out undefined/null points and invalid polygons
        const sanitizedPolygons = uniquePolygons
            .map(polygon => polygon.filter(p => p && typeof p[0] === 'number' && typeof p[1] === 'number'))
            .filter(polygon => polygon.length >= 3);

        const polygonsToUse = sanitizedPolygons;
    ...
    _cleanPolygon(polygon, epsilon) {
        ...
        polygon.forEach((pt) => {
            if (!pt) return; // Skip null/undefined points
            const x = Array.isArray(pt) ? Number(pt[0]) : (pt && typeof pt.x === 'number' ? Number(pt.x) : NaN);
            const y = Array.isArray(pt) ? Number(pt[1]) : (pt && typeof pt.y === 'number' ? Number(pt.y) : NaN);
    ...
    calculatePolygonArea(polygon) {
        ...
        // Filter out undefined/null points first
        const validPolygon = polygon.filter(p => p && typeof p[0] === 'number' && typeof p[1] === 'number');
        if (validPolygon.length < 3) return 0;

        let area = 0;
        for (let i = 0; i < validPolygon.length; i++) {
            const j = (i + 1) % validPolygon.length;
            area += validPolygon[i][0] * validPolygon[j][1];
            area -= validPolygon[j][0] * validPolygon[i][1];
        }
    ...
    calculateCentroid(polygon) {
        ...
        // Filter out undefined/null points first
        const validPolygon = polygon.filter(p => p && typeof p[0] === 'number' && typeof p[1] === 'number');
        if (validPolygon.length < 3) return { x: 0, y: 0 };

        // Weighted centroid using area
        let sumX = 0, sumY = 0, totalArea = 0;
        for (let i = 0; i < validPolygon.length; i++) {
            const j = (i + 1) % validPolygon.length;
            const factor = (validPolygon[i][0] * validPolygon[j][1] - validPolygon[j][0] * validPolygon[i][1]);
            sumX += (validPolygon[i][0] + validPolygon[j][0]) * factor;
            sumY += (validPolygon[i][1] + validPolygon[j][1]) * factor;
            totalArea += factor;
        }
    ...
    getPolygonBounds(polygon) {
        ...
        polygon.forEach(pt => {
            if (!pt || typeof pt[0] !== 'number' || typeof pt[1] !== 'number') return;
