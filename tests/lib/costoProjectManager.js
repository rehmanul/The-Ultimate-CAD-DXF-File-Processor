...
class CostoProjectManager {
    constructor() {
        // Don't create directory at initialization - do it lazily when needed
        this.projectsDir = null;
        this._initialized = false;
    }
    
    _ensureProjectsDir() {
        if (this._initialized && this.projectsDir) {
        ...
        // If we get here, couldn't create any directory
        console.warn('[CostoProjectManager] Could not create projects directory, operations will fail');
        this.projectsDir = possibleDirs[0]; // Use first as fallback
        this._initialized = true;
        return this.projectsDir;
    }
    ...
    saveProject(projectId, projectData) {
        ...
        const prevVersion = existing?.version ? parseFloat(existing.version) : 0;
        const nextVersion = (Number.isFinite(prevVersion) ? prevVersion : 0) + 0.1;
        const versionStr = nextVersion.toFixed(1);
        ...
        const project = {
            ...
            versionId: `v${versionStr}_${Date.now()}`,
