...
let dbFile = null;
const FORCE_SQLJS = ['1', 'true', 'yes', 'on'].includes(
    String(process.env.SQLITE_ADAPTER_FORCE_SQLJS || '').toLowerCase()
);

function shouldSkipBetterSqlite() {
    if (FORCE_SQLJS) return true;
    const major = Number.parseInt((process.versions && process.versions.node || '0').split('.')[0], 10);
    // better-sqlite3 v8 in this project is unstable on very new Node runtimes.
...
async function init(options = {}) {
    dbFile = options.dbFile || path.join(DEFAULT_DB_DIR, options.fileName || 'floorplan.db');

    if (isBetterAvailable() && !shouldSkipBetterSqlite()) {
        try {
            ...
            console.warn('better-sqlite3 initialization failed; attempting sql.js:', e.message);
        }
    }
    ...
    try {
        ...
        db = null;
        emitter.emit('ready');
        throw new Error(`SQLite adapter unavailable: ${e && e.message ? e.message : e}`);
    }
}
